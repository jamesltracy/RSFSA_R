...
## Takes 7 min for AIC rerank GLM Binomial Quad
####################################################################################
t1 <- Sys.time()
...
#  system.time(GLMFiltSubsetEvalStats.df <- GLMBinomialQuadOrthMultiSubset.WrapperTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, OutDirectIn))
  ...
  ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
    ...
    OutDirectpsaOld <- gsub("AIC", "AUC", OutDirectpsaOld)
    OutDirect2Old <- paste0(OutDirectpsaOld, "/", output2)
    setwd(OutDirect2Old)
...
OutDirectpsaGLMAUC <- gsub("AIC", "AUC", OutDirectpsaGLMAUC)
    ...
    for(RankType in RankTypeNames) {
      ...
      for(RandomType in RandomTypeNames) {
        ...
        if(StatType==StatTypeNames[1]) {
          ...
          ncol(GLMTestStats)
          head(GLMTestStats)
          GLMTestStats <- GLMTestStats[,c(1:9, 22, 10:21)]
