...
#source("GLMBinomialQuadSubset_GridTrainTestEvalAICc_AUCpa_Calib_Function.R")
#source("GLMSubset_TrainTestEvalK_Function.R")
source("GLMBinomialQuadOrthMultiSubset_WrapperTrainEvalAICc_Function.R")
source("GLMBinomialQuadOrthMultiSubset_WrapperTestEvalAICc_Function.R")
source("GLMBinomialQuadOrthMultiSubset_WrapperTrainTestEvalAICc_Function.R")
source("GLMBinomialQuadOrthMultiSubset_FinalTrainTestEvalAICc_Function.R")
...
ModelName <- paste0("GLMBinQuad", DataSet, "_", AnalysisType)
...
#####
####################################################################################################
### For quadratic models, orthogonalize each variable using poly function and save orthogonalization
### coefficients for each variable to call for orthogonalization in model training and prediction
### for both matrix and raster data
### NOTE: This section only needs to be run once
####################################################################################################
### Combine all background, pseudoabsence and presence data together for orthogonalization
###
#AllDataDat.df <- rbind(BackgroundDat.df, PseudoabsenceDat.df, PresenceDat.df)
#### Orthogonalize each variable using poly function and save orthogonalization coefficients list for
### later reference
#head(AllDataDat.df)
#polycoefs.list <- list()
#countt <- 0
#setwd(InDirect)
########################################
#for(t in 4:(ncol(AllDataDat.df))) {
#  #t=4
#  polyx <- poly(AllDataDat.df[,t], 2)
#  head(polyx)
#  polycoefs <- attr(polyx, "coefs")
#  #str(polycoefs)
#  # Save orthogonalization coefficients for variable
#  polyvar <- colnames(AllDataDat.df)[t]
#  polycoefs.df1 <- setNames(data.frame(c(polycoefs$alpha,0,0)), paste0(polyvar, "_alpha"))
#  polycoefs.df2 <- setNames(data.frame(polycoefs$norm2), paste0(polyvar, "_norm2"))
#  polycoefs.df <- cbind(polycoefs.df1, polycoefs.df2)
#  countt <- countt + 1
#  polycoefs.list[[countt]] <- polycoefs.df
#}
#####
#polycoefsall.df <- do.call(cbind, polycoefs.list)
#polycoefsall.df[,1:12]
#write.csv(polycoefsall.df, file=paste0(Species,"_PolyOrthogonalizationCoef.csv"))
## Check read
##polycoefsall.df <- read.csv(paste0(Species, "_PolyOrthogonalizationCoef.csv"), stringsAsFactors=FALSE)
### Keep only columns with variable name
##polycoef.df <- polycoefsall.df[ , grepl(polyvar, names(polycoefsall.df))]
### Transfer coefficients to properly formatted list
##polycoeflist <- list()
...
## Takes 15 min for GLM Binomial Quad (using makeglm)
####################################################################################
t1 <- Sys.time()
...
for(SubsetLoop in SubsetSizes) {
  ...
  ### Execute only if reading in VariableSubsetsIn from another RSFSA run (e.g., MaxEnt)
    OutDirectpsaOld <- gsub("GLMBinQuad", "Maxent", OutDirectpsa)
    OutDirectpsaOld <- gsub("SWNAR4O", "SWNAR3", OutDirectpsaOld)
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  system.time(GLMFiltSubsetEvalStats.df <- GLMBinomialQuadOrthMultiSubset.WrapperTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, OutDirectIn))
  #
  ######################
  ...
  for (j in 1:2) {
    ...
    ##
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GLMTrainSubsetEvalStats.df <- GLMBinomialQuadOrthMultiSubset.WrapperTrainEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, OutDirectIn, DataSetType=DataSetType))
  ...
  for (j in 1:2) {
    ...
    DataSetType <- DataSetTypes[j]
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GLMTrainTestSubsetEvalStats.df <- GLMBinomialQuadOrthMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, OutDirectIn, DataSetType=DataSetType))
    ## Separate out training and testing data
...
OutDirectpsaGLMAUC <- gsub("GLMBinQuad", "GLMBinomial", OutDirectpsa)
OutDirectpsaGLMAUC <- gsub("SWNAR4O", "SWNAR4", OutDirectpsaGLMAUC)
...
#    ####################
    for(SetRun in SetRunsList) {
      ...
      SetCount <- SetCount + 1
      SetRunID <- SetCount
      system.time(GLMWrapperTrainTestSubsetEvalStats.df1 <- GLMBinomialQuadOrthMultiSubset.WrapperTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, OutDirectIn, FunctDirectIn))
    }
    ##############################
...
#    setwd(OutDirect2Orig)
    ...
    for(ChosenModSets in ChosenModSetSizes) {
      ...
      for (j in 1:length(DataSetTypes)) {
        ...
        DataSetType <- DataSetTypes[j]
        ## Evaluate random subsets for NumberModSets number of subsets
        system.time(GLMTrainTestSubsetEvalStats.df1 <- GLMBinomialQuadOrthMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
        # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
    ...
    for(RankType in RankTypeNames) {
      ...
      for(RandomType in RandomTypeNames) {
        ...
        if(StatType==StatTypeNames[1]) {
          ...
          ncol(GLMTestStats)
          head(GLMTestStats)
          GLMTestStats <- GLMTestStats[,c(1:9, 23, 10:22)]
