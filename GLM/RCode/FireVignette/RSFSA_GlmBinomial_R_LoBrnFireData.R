...
## Takes 4.8 hrs for GLM Binomial Quad
## Takes 36 min for GLM Binomial
####################################################################################
t1 <- Sys.time()
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  ...
  for (j in 1:2) {
    ...
    #
    GLMSubsetEvalStats.df <- do.call(rbind, list(GLMTrainSubsetEvalStats.df, GLMTestSubsetEvalStats.df))
    GLMSubsetEvalStats.df$SetName <- SetName
    GLMSubsetEvalStats.df$Set <- DataSetType
    GLMSubsetEvalStats.df$TotSubsets <- Sets
...
for(SubsetLoop in SubsetSizes) {
  ...
  OutDirect2 <- paste0(OutDirectpsa, "/", output2)
  setwd(OutDirect2)
  GLMEvalStats.df <- data.frame(read.csv(paste0(Species, "GLMResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", 250, "_.csv"), stringsAsFactors=FALSE))
  head(GLMEvalStats.df)
  Count <- Count + 1
  GLMEvalStatsL[[Count]] <- GLMEvalStats.df
}
################
GLMWrapperTestEvalStats.df <- do.call(rbind, GLMEvalStatsL)
head(GLMWrapperTestEvalStats.df)
tail(GLMWrapperTestEvalStats.df)
#####################
# Regress AICc_bg agains AICc2
AICMod <- lm(AICc_bg ~ AICc2, data=GLMWrapperTestEvalStats.df)
#str(AICMod)
# Retrieve Y intercept
Intercept <- AICMod$coefficients[1]
AICc2_Coef <- AICMod$coefficients[2]
#
AICModStats <- summary(AICMod)
#str(AICModStats)
AICMod_AdjRSq <- AICModStats$adj.r.squared
## Add twice the absolute value of the intercept to AICc_bg to get AICc_bg_adj
GLMWrapperTestEvalStats.df$AICc_bg_adj <- GLMWrapperTestEvalStats.df$AICc_bg + (2 * abs(Intercept))
head(GLMWrapperTestEvalStats.df)
# Drop first column of row names and drop NumEnVars
GLMWrapperTestEvalStats.df <- GLMWrapperTestEvalStats.df[,c(-1)]
# Save file
setwd(OutDirectpsa)
write.table(GLMWrapperTestEvalStats.df, file=paste0(Species, "GLMResults_WrapperTest_", TotVars, "TotVars_", min(SubsetSizes), "to", max(SubsetSizes), "Vars_", 250, "_.csv"), sep=",", row.names=FALSE)
# Save adjusted r-squared for regression
AICModelStats.df <- data.frame(t(as.matrix(c(0,0,0,0,0))))
colnames(AICModelStats.df) <- c("SubsetVariableNumber", "N", "Intercept", "AICc2_Coef", "AdjRSq")
#
AICModelStats.df$SubsetVariableNumber <- SubsetVariableNumber
AICModelStats.df$N <- NumberModSets
AICModelStats.df$Intercept <- as.vector(Intercept)
AICModelStats.df$AICc2_Coef <- as.vector(AICc2_Coef)
AICModelStats.df$AdjRSq <- AICMod_AdjRSq
# Save AICc_bg Vs AICc2 regression stats for each SubsetVariableNumber
setwd(OutDirectpsa)
write.table(AICModelStats.df, file=paste0(Species, "_AllWrapperTest_AICc_bg_Vs_AICc2_RegressionStats.csv"), sep=",", row.names=FALSE)

################################################################################
### Calculate AICc_bg_adj for each SubsetVariableNumber and save to data set
...
# Extract intercept value for SubsetVariableNumber
Intercept <- AICModelStats.df$Intercept
## Add twice the absolute value of the intercept to AICc_bg to get AICc_bg_adj
GLMKeepEvalStats.df$AICc_bg_adj <- GLMKeepEvalStats.df$AICc_bg + (2 * abs(Intercept))
## Set AICc_bg_adj values to zero where NumEnVars is zero
GLMKeepEvalStats.df2 <- within(GLMKeepEvalStats.df, AICc_bg_adj[NumEnVars==0] <- 0)
head(GLMKeepEvalStats.df2)
tail(GLMKeepEvalStats.df2)
# Drop first column of row names
GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df2[,c(-1)]
ncol(GLMKeepEvalStats.df2)
# Move the NumDVars and NumEnVars to end
GLMKeepEvalStats.df3 <- GLMKeepEvalStats.df2[,c(1:4, 7:21, 5:6)]
head(GLMKeepEvalStats.df3)
## Save revised data
setwd(OutDirectpsa)
write.table(GLMKeepEvalStats.df3, file=paste0(Species, ModelName, NObs, "_FullSetTrainVsTestStats_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff2.csv"), sep=",", row.names=FALSE)


###########################
### Calculate statistic for new data set including AICc_bg_adj
### Summarize Output
###Calculate Mean and Standard Deviation Values
RunType <- "Final"
EvaluationStats <- GLMKeepEvalStats.df3
