GLMBinomialQuadMultiSubset.WrapperTestEvalAICc <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, FunctDirectIn..., Output) {
  library(AICcmodavg)
  library(dismo)
  library(foreach)
  library(doParallel)
  ...
  GLMSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    function(CoefficientsIn, xData.dfIn) {
      ...
      FakeYTrainData <- as.factor(sample(0:1, nrow(xData.dfIn), replace=T))
    ...
    for(i in 1:ncol(xTrainData.df)) {
      #i=3
      # Use poly function to obtain raw main effect and squared quadratic value
