GLMBinomialQuadMultiSubset.WrapperTrainEvalAICc <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, FunctDirectIn..., DataSetType, Output) {
  setwd(OutDirectIn)
  library(AICcmodavg)
  library(dismo)
  library(foreach)
  ...
  GLMSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    k=1
    # Specify model testing data and add count column as factor
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      write.table(GLMSubsetEvalStats.df, file=paste0(Species, "GLMResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, ".csv"), sep=",", col.names=NA)
    } else {
      write.table(GLMSubsetEvalStats.df, file=paste0(Species, "GLMResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, ".csv"), sep=",", col.names=NA)
    }
    #
    out5 <- paste("\n  GLM Response")
