GLMBinomialQuadOrthMultiSubset.WrapperTestEvalAICc <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, OutDirectIn, FunctDirectIn..., Output) {
  library(AICcmodavg)
  library(dismo)
  library(foreach)
  library(doParallel)
  ...
  #
  ModelType1 <- paste("GLMBinQuad", Subset, sep="")
  Model <- paste("GLMBinQuad for", SubsetVariableNumber, "Feature Subset")
  ...
  GLMSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    function(CoefficientsIn, xData.dfIn) {
      ...
      for(m in CoefficientsVector) {
        #i=2
        if(m==1) {
          CoefficientsVector[m] <- paste(Coefficients.df[1,1])
        } else {
          CoefficientsVector[m] <- paste0(colnames(Coefficients.df)[m], "=", Coefficients.df[1,m])
        }
      }
      ###########
  ...
  if(Output==TRUE) {
    ...
    write.table(GLMSubsetEvalStats.df, file=paste0(Species, "GLMResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    #
    out5 <- paste("\n  GLM Response")
