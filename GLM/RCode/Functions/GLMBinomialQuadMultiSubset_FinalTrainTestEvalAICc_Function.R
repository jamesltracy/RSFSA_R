GLMBinomialQuadMultiSubset.FinalTrainTestEvalAICc <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, FunctDirectIn..., SetRunIDIn, Output, DataSetType) {
  setwd(OutDirectIn)
  library(AICcmodavg)
  library(dismo)
  library(foreach)
  ...
  #
  ModelType1 <- paste("GLMBinomialQuad", Subset, sep="")
  Model <- paste("GLMBinomialQuad for", SubsetVariableNumber, "Feature Subset")
  ...
  GLMPresTrainData <- PresenceDat.df[kfoldgrpp==3,]
  GLMPresTrainData$Count <- 1
  head(GLMPresTrainData)
  #str(GLMPresTrainData)
  GLMAbsTrainData <- PseudoabsenceDat.df[kfoldgrpp==3,]
  ...
  GLMSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    for(n in 1:ncol(xTrainData.df)) {
      #n=3
      # Use poly function to obtain raw main effect and squared quadratic values
      xTrainDataQ.df[,colcount:(colcount+1)] <- poly(xTrainData.df[,n], 2, raw=TRUE)
      head(xTrainDataQ.df)
    ...
    head(xTrainDataQ.df)
    # Replace "^" with "P"
    colnames(xTrainDataQ.df) <- gsub("\\^2", "P2", colnames(xTrainDataQ.df))
    #####################
    ...
    fla <- paste("yTrainData ~", paste(colnames(xTrainDataQ.df), collapse=" + "))
    # Run GLM
    GLMBinomialModel <- glm(as.formula(fla), data=xTrainDataQ.df, family="binomial")
    ...
    for(m in 1:ncol(xPresData.dfIn)) {
      #m=3
      # Use poly function to obtain raw main effect and squared quadratic values
      xPresDataQ.dfIn[,colcount:(colcount+1)] <- poly(xPresData.dfIn[,m], 2, raw=TRUE)
    ...
    presvals.df <- setNames(data.frame(predict(GLMBinomialModel, xDataQ.dfIn, type="response")), "GLMScore")
    head(presvals.df)
    ...
    for(m in 1:ncol(xData.dfIn)) {
      #m=3
      # Use poly function to obtain raw main effect and squared quadratic values
      xDataQ.dfIn[,colcount:(colcount+1)] <- poly(xData.dfIn[,m], 2, raw=TRUE)
      head(xDataQ.dfIn)
    ...
    head(xDataQ.dfIn)
    #####################
    backgroundvals.df <- setNames(data.frame(predict(GLMBinomialModel, xDataQ.dfIn, type="response")), "GLMScore")
    ...
    for(TestDataType in TestDataTypes) {
      ...
      head(GLMPresTestData)
      # Subset PresTestData by VarNames
      xData.dfIn <- GLMPresTestData[, c(VarsUsed), drop=FALSE]
      head(xData.dfIn)
      #####################
      ## Add orthogonalized quadratic versions for main effect variables (no interaction)
      ##
      ...
      prespred.df <- setNames(data.frame(predict(GLMBinomialModel, xDataQ.dfIn, type="response")), "GLMScore")
      head(prespred.df)
      ...
      head(GLMAbsTestData)
      # Subset PresTestData by VarNames
      xData.dfIn <- GLMAbsTestData[, c(VarsUsed), drop=FALSE]
      head(xData.dfIn)
      #
      #####################
      ## Add orthogonalized quadratic versions for main effect variables (no interaction)
      ...
      head(xDataQ.dfIn)
      #####################
      abspred.df <- setNames(data.frame(predict(GLMBinomialModel, xDataQ.dfIn, type="response")), "GLMScore")
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      if(SetRunIDIn!="") {
        write.table(GLMSubsetEvalStats.df, file=paste0(Species, "GLMResults_TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, "_", SetRunIDIn, ".csv"), sep=",", col.names=NA)
      } else {
        write.table(GLMSubsetEvalStats.df, file=paste0(Species, "GLMResults_TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, ".csv"), sep=",", col.names=NA)
      }
    } else {
      if(SetRunIDIn!="") {
        write.table(GLMSubsetEvalStats.df, file=paste0(Species, "GLMResults_FinalTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunIDIn, ".csv"), sep=",", col.names=NA)
      } else {
        write.table(GLMSubsetEvalStats.df, file=paste0(Species, "GLMResults_FinalTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, ".csv"), sep=",", col.names=NA)
      }
    }
    out5 <- paste("\n  GLM Response")
