...
# GLM niche modelling described in Tracy et al. (in prep.)
#
...
#source("GLMSubset_TrainTestEval_Function.R")
source("GLMBinomialSubset_GridTrainTestEvalAICc_AUCpa_Calib_Function.R")
#source("GLMSubset_TrainTestEvalK_Function.R")
source("GLMBinomialMultiSubset_WrapperTrainEvalAICc_Function.R")
source("GLMBinomialMultiSubset_WrapperTestEvalAICc_Function.R")
source("GLMBinomialMultiSubset_WrapperTrainTestEvalAICc_Function.R")
source("GLMBinomialMultiSubset_FinalTrainTestEvalAICc_Function.R")
#source("GLMProject_Function.R")
#source("GLMProjectRAW_AICc_Function.R")
...
library(MCDM)
library(dplyr)
#install_github('johnbaums/rGLM')  # for installing rGLM
#library(rGLM)
#library(rasterVis)
#library(viridis)
...
## Specify base argumenets for GLM settings for evaluation run
alphain <- 1 # 1 for lasso L1 regularization; 2 for ridge regression L2 regularization
cvGLMfoldsin <- 5 # number of folds for GLM cross validation run
#
...
## Specify location of temporary directory where R stores GLM output if want
## deleted to avoid accumulation of many GLM output folders when running features selection algorithm
#TempDir <- "C:/Users/james/AppData/Local/Temp/raster/GLM"
TempDir <- ""
## Specify extent and various descriptors for GLM run
Extent <- "SWNA"
...
SpecFileName <- "mdsevfreq1"
Species <- paste0("ModBrn")
SpecShort <- paste0("MdBr")
SpeciesGen <- "Fire"
...
Tag <- "R4"
LongSpecies <- "MdBurnSeverityLessorEqualto16yrInterval"
ClassRaster <- "mtlfallmdsumr"  # name of classification raster of burn frequencies
...
ModelName <- paste0("GLMBinomial", DataSet, "_", AnalysisType)
...
#### NOTE: GLM prediction does not work with R grids, must use Arc Grid or Arc Ascii Grid
######################################################################
#setwd(MaskGrids)
...
##
PseudoabsenceDat.df <- data.frame(read.csv(paste0(Species, "_", PsAbsBuff, Units, "PsAbsBuff_pseudoabsevaldat", DataSet, TagIn, "_", TotVars, "Vars.csv"), sep=",", row.names=1, stringsAsFactors=FALSE))
...
## Takes 36 min for GlmMnet Binomial
## Takes 17 min for GLM Binomial
####################################################################################
t1 <- Sys.time()
GLMKeepEvalAllStatsL <- list()
GLMKeepEvalAllStatSummL <- list()
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  system.time(GLMFiltSubsetEvalStats.df <- GLMBinomialMultiSubset.WrapperTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn))
  #
  ######################
  ...
  Sets <- NumberModSets
  TestDataType <- "Wrapper"
  GLMFiltSubsetEvalStats.df <- data.frame(read.csv(paste0(Species, "GLMResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunID, ".csv"), sep=",", row.names=1, stringsAsFactors=FALSE))
  head(GLMFiltSubsetEvalStats.df)
  tail(GLMFiltSubsetEvalStats.df)
  # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
  GLMFiltSubsetEvalStats.df2 <- subset(GLMFiltSubsetEvalStats.df, AICc2!="Inf")
  NewSets1 <- nrow(GLMFiltSubsetEvalStats.df2)
  ## Use Multi-Objective Optimization from MCDM package to sort by two criteria of AUC and AICc_bg
  decision.mat <- as.matrix(GLMFiltSubsetEvalStats.df2[,c(13,7)])
  head(decision.mat)
  # Normalize AICc and AUC from zero to 1
  ...
  head(MMOORA.Rank)
  GLMFiltSubsetEvalStats.df4 <- GLMFiltSubsetEvalStats.df2
  # Join MMOORA rank to original data
  GLMFiltSubsetEvalStats.df4$MMOORA_Rank <- MMOORA.Rank[,8]
  head(GLMFiltSubsetEvalStats.df4)
  # Sort data by MMOORA rank
  GLMFiltSubsetEvalStats.df4s <- GLMFiltSubsetEvalStats.df4[order(GLMFiltSubsetEvalStats.df4$MMOORA_Rank),]
  head(GLMFiltSubsetEvalStats.df4s)
  ncol(GLMFiltSubsetEvalStats.df4s)
  # Delete rank column
  GLMFiltSubsetEvalStats.df5 <- GLMFiltSubsetEvalStats.df4s[,-16]
  head(GLMFiltSubsetEvalStats.df5)
  # Keep top NObs selected subsets
  GLMTopNObsFiltSubsetEvalStats.df <- data.frame(GLMFiltSubsetEvalStats.df5[1:NObs,], stringsAsFactors=FALSE)
  GLMTopNObsFiltSubsetEvalStats.df
  TopNObsSubsets <- data.frame(GLMTopNObsFiltSubsetEvalStats.df[,1], stringsAsFactors=FALSE)
  ##############################
  ## NOTE: If comparing algorithm, use same random subsets and comment next three lines
  # Keep random NObs random subsets
  #GLMRandNObsFiltSubsetEvalStats.df <- data.frame(GLMFiltSubsetEvalStats.df2[sample(nrow(GLMFiltSubsetEvalStats.df2), NObs), ], stringsAsFactors=FALSE)
  #RandNObsSubsets <- data.frame(GLMRandNObsFiltSubsetEvalStats.df[,1], stringsAsFactors=FALSE)
  #colnames(RandNObsSubsets) <- "VarNames"
  #################
  ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
    OutDirectpsaOld <- gsub("GLMBinomial", "Maxent", OutDirectpsa)
    OutDirectpsaOld <- gsub("SWNAR4", "SWNAR3", OutDirectpsaOld)
    OutDirect2Old <- paste0(OutDirectpsaOld, "/", output2)
    setwd(OutDirect2Old)
    MaxEntOldEvalStats.df <- data.frame(read.csv(paste0(Species, "MaxentResults_Filter_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_Random", NObs, "_", NObs, ".csv"), stringsAsFactors=FALSE))
    RandNObsSubsets <- MaxEntOldEvalStats.df[,2, drop=FALSE]
    head(RandNObsSubsets)
    # Subset GLMFiltSubsetsEvalStats.df2 by RandNObsSubsets
    GLMRandNObsFiltSubsetEvalStats.df <- GLMFiltSubsetEvalStats.df2[GLMFiltSubsetEvalStats.df2$VarNames %in% RandNObsSubsets[,1],]
  ##################
  ...
  for (j in 1:2) {
    ...
    if(DataSetType==paste0("Top", NObs)) {
      GLMTestSubsetEvalStats.df <- GLMTopNObsFiltSubsetEvalStats.df
    } else {
      GLMTestSubsetEvalStats.df <- GLMRandNObsFiltSubsetEvalStats.df
    }
    GLMTestSubsetEvalStats.df$DataType <- paste0(DataSetType, "WrapperTest")
    head(GLMTestSubsetEvalStats.df)
    ##
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GLMTrainSubsetEvalStats.df <- GLMBinomialMultiSubset.WrapperTrainEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, DataSetType=DataSetType))
    ## Separate out training and testing data
    GLMTrainSubsetEvalStats.df$DataType <- paste0(DataSetType, "WrapperTrain")
    head(GLMTrainSubsetEvalStats.df)
    # Calculate difference between test and train statistics for overfitting
    GLMDiffTestTrainSubsetEvalStats.df <- GLMTestSubsetEvalStats.df
    ncol(GLMDiffTestTrainSubsetEvalStats.df)
    GLMDiffTestTrainSubsetEvalStats.df[,5:15] <- GLMTrainSubsetEvalStats.df[,5:15] - GLMTestSubsetEvalStats.df[,5:15]
    GLMDiffTestTrainSubsetEvalStats.df$DataType <- paste0(DataSetType, "WrapperDiffTestTrain")
    head(GLMDiffTestTrainSubsetEvalStats.df)
    #
    GLMSubsetEvalStats.df <- do.call(rbind, list(GLMTrainSubsetEvalStats.df, GLMTestSubsetEvalStats.df, GLMDiffTestTrainSubsetEvalStats.df))
    GLMSubsetEvalStats.df$SetName <- SetName
    GLMSubsetEvalStats.df$Set <- DataSetType
    GLMSubsetEvalStats.df$TotSubsets <- Sets
    GLMSubsetEvalStats.df$PresTrainPoints <- nrow(PresenceDat.df[kfoldgrpp==1,])
    GLMSubsetEvalStats.df$Run <- Run
    GLMSubsetEvalStats.df <- GLMSubsetEvalStats.df[, colnames(GLMSubsetEvalStats.df)[c(1:4,14:20,5:13)]]
    #
    GLMKeepEvalStatsL[[j]] <- GLMSubsetEvalStats.df
  }
  ###############
  ...
  for (j in 1:2) {
    ...
    DataSetType <- DataSetTypes[j]
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GLMTrainTestSubsetEvalStats.df <- GLMBinomialMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, DataSetType=DataSetType))
    ## Separate out training and testing data
    GLMTrainSubsetEvalStats.df <- GLMTrainTestSubsetEvalStats.df[which(GLMTrainTestSubsetEvalStats.df$DataType=="FinalTrain"),]
    GLMTrainSubsetEvalStats.df$DataType <- paste0(DataSetType, "FinalTrain")
    GLMTestSubsetEvalStats.df <- GLMTrainTestSubsetEvalStats.df[which(GLMTrainTestSubsetEvalStats.df$DataType=="FinalTest"),]
    GLMTestSubsetEvalStats.df$DataType <- paste0(DataSetType, "FinalTest")
    # Calculate difference between test and train statistics for overfitting
    GLMDiffTestTrainSubsetEvalStats.df <- GLMTestSubsetEvalStats.df
    GLMDiffTestTrainSubsetEvalStats.df[,5:15] <- GLMTrainSubsetEvalStats.df[,5:15] - GLMTestSubsetEvalStats.df[,5:15]
    GLMDiffTestTrainSubsetEvalStats.df$DataType <- paste0(DataSetType, "FinalDiffTestTrain")
    #
    GLMSubsetEvalStats.df <- do.call(rbind, list(GLMTrainSubsetEvalStats.df, GLMTestSubsetEvalStats.df, GLMDiffTestTrainSubsetEvalStats.df))
    GLMSubsetEvalStats.df$SetName <- SetName
    GLMSubsetEvalStats.df$Set <- DataSetType
    GLMSubsetEvalStats.df$TotSubsets <- Sets
    GLMSubsetEvalStats.df$PresTrainPoints <- nrow(PresenceDat.df[kfoldgrpp==3,])
    GLMSubsetEvalStats.df$Run <- Run
    GLMSubsetEvalStats.df <- GLMSubsetEvalStats.df[, colnames(GLMSubsetEvalStats.df)[c(1:4,14:20,5:13)]]
    #
    GLMKeepEvalStatsL2[[j]] <- GLMSubsetEvalStats.df
  }
  ###############
  GLMKeepEvalStats.df1 <- do.call(rbind, GLMKeepEvalStatsL)
  GLMKeepEvalStats.df2 <- do.call(rbind, GLMKeepEvalStatsL2)
  GLMKeepEvalStats.df3 <- rbind(GLMKeepEvalStats.df1, GLMKeepEvalStats.df2)
  GLMKeepEvalAllStatsL[[OutCount]] <- GLMKeepEvalStats.df3
  ### Summarize Output
  ###Calculate Mean and Standard Deviation Values
  RunType <- "Final"
  EvaluationStats <- GLMKeepEvalStats.df3
  ...
  GLMKeepEvalAllStatSummL[[OutCount]] <- EvaluationStatsSummary.df
  ######
  GLMKeepEvalStatsAllSets.df <- do.call(rbind, GLMKeepEvalAllStatSummL)
  ncol(GLMKeepEvalStatsAllSets.df)
  ## Save data
  write.table(GLMKeepEvalStatsAllSets.df, file=paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, ".csv"), sep=",", col.names=NA)
  ### Summarize and save raw data as loop progresses
  GLMKeepEvalStats.df <- do.call(rbind, GLMKeepEvalAllStatsL)
  # Save data
  setwd(OutDirectpsa)
  write.table(GLMKeepEvalStats.df , file=paste0(Species, ModelName, NObs, "_FullSetTrainVsTestStats_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff.csv"), sep=",", col.names=NA)
}
#
####################################
t2 <- Sys.time()
...
GLMKeepEvalSumStats.df1 <- data.frame(read.csv(paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, ".csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
## Sort data by Statistic and then by DataType for ease of plotting in excel
GLMKeepEvalSumStats.dfs1 <- arrange(GLMKeepEvalSumStats.df1, Statistic, DataType)
GLMKeepEvalSumStats.dfs <- arrange(GLMKeepEvalSumStats.dfs1 , DataType)
head(GLMKeepEvalSumStats.dfs)
#
write.table(GLMKeepEvalSumStats.dfs, file=paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, ".csv"), sep=",", col.names=NA)
...
#SubsetSizes <- c(3,6,8,10,12,15,20,25)
#
GLMKeepEvalStats.df1 <- data.frame(read.csv(paste0(Species, ModelName, NObs, "_FullSetTrainVsTestStats_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
nrow(GLMKeepEvalStats.df1)
ncol(GLMKeepEvalStats.df1)
head(GLMKeepEvalStats.df1)
tail(GLMKeepEvalStats.df1)
# Keep only DataTypes including string "FinalTest"
GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df1[grep("FinalTest", GLMKeepEvalStats.df1$DataType), ]
GLMKeepEvalStats.df2[1:50,]
tail(GLMKeepEvalStats.df2)
# Create DataTypeN combining SubsetVariableNumber and DataType
GLMKeepEvalStats.df2$DataTypeN <- paste0(GLMKeepEvalStats.df2$DataType, "_",GLMKeepEvalStats.df2$SubsetVariableNumber)
head(GLMKeepEvalStats.df2)
ncol(GLMKeepEvalStats.df2)
# Merge SubsetVariableNumber with SetName
GLMKeepEvalStats.df2$SetName <- paste0(GLMKeepEvalStats.df2$SetName, "_", as.character(GLMKeepEvalStats.df2$SubsetVariableNumber))
# Reorder Columns
GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df2[,c(21,1:20)]
###Calculate Mean and Standard Deviation Values
RunType <- "Final"
EvaluationStats <- GLMKeepEvalStats.df2
# Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
EvaluationStats <- subset(EvaluationStats, AICc2!="Inf")
# Replace NAN values with zero
EvaluationStats[is.na(EvaluationStats)] <- 0
#
OutName <- paste0(Species, RankStatistic, "GLM_VariousSetsof_", SubsetVariableNumber, "_", NObs, "N_FinalTestStatSummary.csv")
...
head(EvalStatsOuts)
write.table(EvalStatsOuts, file=paste0(Species, RankStatistic, "GLMFullSet", NObs, "StatSetRunsbySubsetSizes_FinalTestSummaryTable.csv"), sep=",", col.names=NA)
##
#

##################################################################################
### Calculate AICc_bg estimated by regression (AICc_bg_reg) for comparison with MaxEnt.
### Pooling data for all SubsetVariableNumbers, regress AICc_bg for linear Maxent models
### against AICc2 for GLM for models using the same number of derived variables
### (assuming AICc values should be equal) and obtain regression formula for estimating
### AICc_bg from AICc2 for other GLM models and SubsetVariableNumbers
#################################################################################
##
## Loop through each SubsetVariableNumber and collect all data for regression
SubsetSizes <- c(3, 6, 8, 10, 12, 15, 20, 25)
NumberModSets <- 250
GLMLinearEvalStatsL <- list()
VariableNamesIn <- VariableNames
Count <- 0
################################################################
for(SubsetLoop in SubsetSizes) {
  ...
  TotVars <- nrow(VariableNamesIn)
  output2 <- paste0(NumberModSets,"Setsof", Subset, SubsetVariableNumber, "of", TotVars, "Vars")
  ## First read in GLM stats
  OutDirect2 <- paste0(OutDirectpsa, "/", output2)
  setwd(OutDirect2)
  GLMLinearEvalStats.dfa <- data.frame(read.csv(paste0(Species, "GLMResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", 250, "_.csv"), stringsAsFactors=FALSE))
  head(GLMLinearEvalStats.dfa)
  ## Next read in corresponding linear MaxEnt stats
  OutDirectpsaOld <- gsub("GLMBinomial", "Maxent", OutDirectpsa)
  OutDirectpsaOld <- gsub("SWNAR4", "SWNAR3L", OutDirectpsaOld)
  OutDirect2Old <- paste0(OutDirectpsaOld, "/", output2)
  setwd(OutDirect2Old)
  MaxentLinearEvalStats.dfa <- data.frame(read.csv(paste0(Species, "MaxentResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", 250, "_.csv"), stringsAsFactors=FALSE))
  head(MaxentLinearEvalStats.dfa)
  ## Merge selected columns of MaxentLinearEvalStats.df with GLMLinearEvalStats.df
  ## First select needed data from Maxent
  MaxentLinearEvalStats.dfs <- MaxentLinearEvalStats.dfa[,c(2, 12,13), drop=FALSE]
  head(MaxentLinearEvalStats.dfs)
  colnames(MaxentLinearEvalStats.dfs)[2:3] <- c("AICc_bgMxt", "NumDVarsMxt")
  ## Merge GLM and Maxent data by VarNames (should match)
  GLMLinearEvalStats.dfb <- merge(GLMLinearEvalStats.dfa, MaxentLinearEvalStats.dfs, by.x="VarNames", by.y="VarNames")
  head(GLMLinearEvalStats.dfb)
  ## Keep only instances of NumDVars = NumDVarsMxt
  GLMLinearEvalStats.dfc <- GLMLinearEvalStats.dfb[which(GLMLinearEvalStats.dfb$NumDVars==GLMLinearEvalStats.dfb$NumDVarsMxt),]
  head(GLMLinearEvalStats.dfc)
  nrow(GLMLinearEvalStats.dfc)
  ## Store data of loop in lists
  Count <- Count + 1
  GLMLinearEvalStatsL[[Count]] <- GLMLinearEvalStats.dfc
}
#############################################
GLMLinearEvalStats.df <- do.call(rbind, GLMLinearEvalStatsL)
head(GLMLinearEvalStats.df)
tail(GLMLinearEvalStats.df)
#####################
## Save data for charting
setwd(OutDirectpsa)
write.table(GLMLinearEvalStats.df, file=paste0(Species, "GLM_Maxent_LinearAICResults_Wrapper_", TotVars, "TotVars_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_.csv"), sep=",", row.names=FALSE)
##
# Run regression of AICc_bg_Mxt against AICc2 of GLM
#polyfitAIC <- lm(AICc_bgMxt ~ poly(AICc2, 2, raw=TRUE), data=GLMLinearEvalStats.df)
fitAIC <- lm(AICc_bgMxt ~ AICc2, data=GLMLinearEvalStats.df)
summary(fitAIC)
# Save GLM model
setwd(OutDirectpsa)
saveRDS(fitAIC, "LinearAICc2toAICc_bgModel.rds")
# Plot results
# First plot raw data
# save following plot as .bmp file
bmp(paste(Species, "AICcCalibrationMaxEntVsGLMPlot", ".bmp", sep=""))
#dev.new()
plot(GLMLinearEvalStats.df$AICc2, GLMLinearEvalStats.df$AICc_bgMxt, col='deepskyblue4', xlab='AICc2', ylab='AICc_bg_Mxt', main='AICc Calibration for MaxEnt Vs GLM')
# Then plot regression
newdat = data.frame(AICc2 = seq(min(GLMLinearEvalStats.df$AICc2), max(GLMLinearEvalStats.df$AICc2), 0.01))
newdat$pred = predict(fitAIC, newdata=newdat)
head(newdat)
nrow(newdat)
with(newdat, lines(x=AICc2, y = pred, col='firebrick1', lwd=3))
dev.off()
#############################
#
# Save regression line values
# Retrieve Y intercept and coefficient
Intercept <- fitAIC$coefficients[1]
AICc2_Coef <- fitAIC$coefficients[2]
#
fitAICStats <- summary(fitAIC)
#str(fitAICStats)
fitAIC_AdjRSq <- fitAICStats$adj.r.squared
# Save adjusted r-squared for regression
fitAICStats.df <- data.frame(t(as.matrix(c(0,0,0,0))))
colnames(fitAICStats.df) <- c("N", "Intercept", "AICc2_Coef", "AdjRSq")
#
fitAICStats.df$N <- nrow(GLMLinearEvalStats.df)
fitAICStats.df$Intercept <- as.vector(Intercept)
fitAICStats.df$AICc2_Coef <- as.vector(AICc2_Coef)
fitAICStats.df$AdjRSq <- fitAIC_AdjRSq
# Save AICc_bg Vs AICc2 regression stats for each SubsetVariableNumber
setwd(OutDirectpsa)
write.table(fitAICStats.df, file=paste0(Species, "_AllWrapperTest_AICc_bg_Vs_AICc2_RegressionStats.csv"), sep=",", row.names=FALSE)

################################################################################
### Calculate AICc_bg_reg save to data set
## Read in the statistics for sets of 10 top and 10 random models for wrapper and final data sets
setwd(OutDirectpsa)
GLMKeepEvalStats.df <- data.frame(read.csv(paste0(Species, ModelName, NObs, "_FullSetTrainVsTestStats_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff.csv"), stringsAsFactors=FALSE))
head(GLMKeepEvalStats.df)
## Calculate AICc_bg_reg using regression equation
xDatIn <- data.frame(AICc2 = GLMKeepEvalStats.df$AICc2)
head(xDatIn)
GLMKeepEvalStats.df$AICc_bg_reg <- predict(fitAIC, newdata=xDatIn)
head(GLMKeepEvalStats.df)
tail(GLMKeepEvalStats.df)
# Drop first column of row names
GLMKeepEvalStats.df <- GLMKeepEvalStats.df[,c(-1)]
ncol(GLMKeepEvalStats.df)
# Move the NumDVars and NumEnVars to end
GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df[,c(1:4, 7:21, 5:6)]
head(GLMKeepEvalStats.df2)
## Save revised data
setwd(OutDirectpsa)
write.table(GLMKeepEvalStats.df2, file=paste0(Species, ModelName, NObs, "_FullSetTrainVsTestStats_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff2.csv"), sep=",", row.names=FALSE)


###########################
### Calculate statistic for new data set including AICc_bg_reg
### Summarize Output
###Calculate Mean and Standard Deviation Values
RunType <- "Final"
EvaluationStats <- GLMKeepEvalStats.df2
ncol(EvaluationStats)
EvaluationStats$Model <- ModelName
EvaluationStats <- EvaluationStats[, colnames(EvaluationStats)[c(22,1:21)]]
head(EvaluationStats)
##
#FSAType <- RankStatistic
OutName <- paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, "2.csv")
SortGroups <- c("Model", "DataType", "Set", "SetName", "SubsetVariableNumber", "TotSubsets")
## All statistics, and only statistics, should be at and after the column specified below
StatVarFirstColumn <- 7
OutDirectIn <- OutDirectpsa
EvaluationStatsSummary.df <- EvalStatVars.Summary(EvaluationStats, SortGroups, StatVarFirstColumn, OutName, OutDirectIn)
head(EvaluationStatsSummary.df)
# Read in data and resort by DataType
GLMKeepEvalStatsNewSummary.df <- data.frame(read.csv(paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, "2.csv"), stringsAsFactors=FALSE))
head(GLMKeepEvalStatsNewSummary.df)
GLMKeepEvalStatsNewSummary.dfs <- arrange(GLMKeepEvalStatsNewSummary.df, DataType)
# Drop first column of row names
GLMKeepEvalStatsNewSummary.dfs <- GLMKeepEvalStatsNewSummary.dfs[,c(-1)]
GLMKeepEvalStatsNewSummary.dfs[1:20,]
write.table(GLMKeepEvalStatsNewSummary.dfs, file=paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, "2.csv"), sep=",", col.names=NA)
...
## Loop takes 30 minutes for 9,0000 NumberModSets of 15 variables for glm binomial low burn severity
###############################################################################
OutCount <- 0
##
GLMKeepEvalAllStatsL <- list()
...
#    ##
    ...
    GLMWrapperTrainTestSubsetEvalStatsL <- list()
    SetCount <- 0
...
#    ####################
    for(SetRun in SetRunsList) {
      ### Execute only if reading in VariableSubsetsIn from another RSFSA run (e.g., MaxEnt)
        ...
        setwd(OutDirect2Old)
        p <- p + 1
        MaxEntOldEvalStats.df <- data.frame(read.csv(paste0(Species, "MaxentResults_FilterTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", RunSize, "_", p, ".csv"), stringsAsFactors=FALSE))
        VariableSubsetsIn <- MaxEntOldEvalStats.df[1:(nrow(MaxEntOldEvalStats.df)/2),2, drop=FALSE]
        head(VariableSubsetsIn)
        setwd(OutDirect2)
      ##################
      ...
      SetCount <- SetCount + 1
      SetRunID <- SetCount
      system.time(GLMWrapperTrainTestSubsetEvalStats.df1 <- GLMBinomialMultiSubset.WrapperTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, FunctDirectIn))
    }
    ##############################
...
#    setwd(OutDirect2Orig)
    ...
    for(SetRunID in SetRunIDList) {
      ...
      NumberSets <- length(SetRunsList[[SetRunID]])
      GLMWrapperTrainTestSubsetEvalStats.df1 <-  as.data.frame(read.csv(paste0(Species, "GLMResults_WrapperTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", NumberSets, "_", SetRunID, ".csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
      tail(GLMWrapperTrainTestSubsetEvalStats.df1)
      # Convert first, second and fourth columns from factor to character
      GLMWrapperTrainTestSubsetEvalStats.df1[,c(1,2,4)] <- sapply(GLMWrapperTrainTestSubsetEvalStats.df1[,c(1,2,4)], function(x) as.character(x))
      Rows <- nrow(GLMWrapperTrainTestSubsetEvalStats.df1)
      #str(GLMWrapperTrainTestSubsetEvalStats.df1)
      #Renumber rows
      if(RowCount==0) {
        RowCount <- Rows
      } else {
        RowCount <- RowCount + Rows
        row.names(GLMWrapperTrainTestSubsetEvalStats.df1) <- seq(RowCount-Rows+1, RowCount, 1)
      }
      GLMWrapperTrainTestSubsetEvalStatsL[[SetRunID]] <- GLMWrapperTrainTestSubsetEvalStats.df1
    }
    GLMWrapperTrainTestSubsetEvalStats.df <- do.call(rbind, GLMWrapperTrainTestSubsetEvalStatsL)
    head(GLMWrapperTrainTestSubsetEvalStats.df)
    ## Separate out training and testing data
    GLMTrainSubsetEvalStats.df <- GLMWrapperTrainTestSubsetEvalStats.df[which(GLMWrapperTrainTestSubsetEvalStats.df$DataType=="WrapperTrain"),]
    GLMTestSubsetEvalStats.df <- GLMWrapperTrainTestSubsetEvalStats.df[which(GLMWrapperTrainTestSubsetEvalStats.df$DataType=="WrapperTest"),]
    # Calculate difference between test and train statistics for overfitting
    GLMDiffTestTrainSubsetEvalStats.df <- GLMTrainSubsetEvalStats.df
    ncol(GLMDiffTestTrainSubsetEvalStats.df)
    GLMDiffTestTrainSubsetEvalStats.df[,5:15] <- GLMTrainSubsetEvalStats.df[,5:15] - GLMTestSubsetEvalStats.df[,5:15]
    GLMDiffTestTrainSubsetEvalStats.df$DataType <- "WrapperDiffTestTrain"
    # Calculate mean between test and train statistics for underfitting
    GLMMeanTestTrainSubsetEvalStats.df <- GLMTrainSubsetEvalStats.df
    ncol(GLMMeanTestTrainSubsetEvalStats.df)
    GLMMeanTestTrainSubsetEvalStats.df[,5:15] <- (GLMTrainSubsetEvalStats.df[,5:15] + GLMTestSubsetEvalStats.df[,5:15])/2
    GLMMeanTestTrainSubsetEvalStats.df$DataType <- "WrapperMeanTestTrain"
    #
    # Join AUCDiff with each data set
    GLMDiffTestTrainSubsetEvalStats.df$AUCdiff <- GLMDiffTestTrainSubsetEvalStats.df$AUC
    GLMTrainSubsetEvalStats.df$AUCdiff <- GLMDiffTestTrainSubsetEvalStats.df$AUC
    GLMTestSubsetEvalStats.df$AUCdiff <- GLMDiffTestTrainSubsetEvalStats.df$AUC
    #
    GLMWrapperTrainTestSubsetEvalStats.df <- do.call(rbind, list(GLMTrainSubsetEvalStats.df, GLMTestSubsetEvalStats.df, GLMDiffTestTrainSubsetEvalStats.df))
    ######################
    ...
    for(ChosenModSets in ChosenModSetSizes) {
      ...
      OutCount <- OutCount + 1
      Run <- 1
      #length(unique(GLMFiltSubsetEvalStats.df$VarNames)) # check for duplicate VarNames
      ## Separate out WrapperTest from data
      GLMFiltSubsetEvalStats.df <- GLMWrapperTrainTestSubsetEvalStats.df[which(GLMWrapperTrainTestSubsetEvalStats.df$DataType=="WrapperTest"),]
      # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
      GLMFiltSubsetEvalStats.df2 <- subset(GLMFiltSubsetEvalStats.df, AICc2="Inf")
      NewSets1 <- nrow(GLMFiltSubsetEvalStats.df2)
      tail(GLMFiltSubsetEvalStats.df2)
      ## Keep only first ChosenModSets number of sets
      GLMFiltSubsetEvalStats.df3 <- GLMFiltSubsetEvalStats.df2[1:ChosenModSets,]
      head(GLMFiltSubsetEvalStats.df3)
      tail(GLMFiltSubsetEvalStats.df3)
      nrow(GLMFiltSubsetEvalStats.df3)
      ## Use Multi-Objective Optimization from MCDM package to sort by two criteria of AUC and AICc_bg
      decision.mat <- as.matrix(GLMFiltSubsetEvalStats.df3[,c(13,7,16)])
      head(decision.mat)
      ...
      RankStatistic <- "AUC"
      AnalysisType <- paste0("RSFSA", RankStatistic)
      ModelName <- paste0("GLM", DataSet, "_", AnalysisType)
      Model <- paste0(ModelName, TotVars, "Feature Subset")
      ##
      AICcrankList <- c(0)
      AUCrankList <- c(1)
      AUCdiffrankList <- c(0)
      GLMTopNObsFiltSubsetEvalStatsL <- list()
      ...
      for(m in 1:length(AUCrankList)) {
        ...
        MMOORA.Rank <- MMOORA(decision.matn, weightscrit, cb)
        head(MMOORA.Rank)
        GLMFiltSubsetEvalStats.df4 <- GLMFiltSubsetEvalStats.df3
        # Join MMOORA rank to original data
        GLMFiltSubsetEvalStats.df4$MMOORA_Rank <- MMOORA.Rank[,8]
        head(GLMFiltSubsetEvalStats.df4)
        # Sort data by MMOORA rank
        GLMFiltSubsetEvalStats.df4s <- GLMFiltSubsetEvalStats.df4[order(GLMFiltSubsetEvalStats.df4$MMOORA_Rank),]
        head(GLMFiltSubsetEvalStats.df4s)
        tail(GLMFiltSubsetEvalStats.df4s)
        nrow(GLMFiltSubsetEvalStats.df4s)
        ncol(GLMFiltSubsetEvalStats.df4s)
        # Delete rank column
        GLMFiltSubsetEvalStats.df5 <- GLMFiltSubsetEvalStats.df4s[,-17]
        head(GLMFiltSubsetEvalStats.df5)
        # Keep top FullNObs selected subsets
        GLMTopNObsFiltSubsetEvalStatsL[[m]] <- data.frame(GLMFiltSubsetEvalStats.df5[1:FullNObs,], stringsAsFactors=FALSE)
        TopNObsSubsetsL[[m]] <- data.frame(GLMFiltSubsetEvalStats.df5[1:FullNObs,1], stringsAsFactors=FALSE)
      }
      #######################
      GLMTopNObsFiltSubsetEvalStats.df <- do.call(rbind, GLMTopNObsFiltSubsetEvalStatsL)
      nrow(GLMTopNObsFiltSubsetEvalStats.df)
      TopNObsSubsets <- do.call(rbind, TopNObsSubsetsL)
      ##############################
      # Keep random FullNObs plus PlusNum random subsets
      GLMRandNObsFiltSubsetEvalStats.df1 <- data.frame(GLMFiltSubsetEvalStats.df3[sample(nrow(GLMFiltSubsetEvalStats.df3), FullNObs+PlusNum), ], stringsAsFactors=FALSE)
      ## Clean random data from aberrant AICc_bg values
      # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
      GLMRandNObsFiltSubsetEvalStats.df2 <- subset(GLMRandNObsFiltSubsetEvalStats.df1, AICc2!="Inf")
      # Keep only FullNObs plus PlusNum/2, or around 50 sets
      GLMRandNObsFiltSubsetEvalStats.df <- GLMRandNObsFiltSubsetEvalStats.df2[1:(FullNObs+Round2(PlusNum/2,0)),]
      tail(GLMRandNObsFiltSubsetEvalStats.df)
      nrow(GLMRandNObsFiltSubsetEvalStats.df)
      ncol(GLMRandNObsFiltSubsetEvalStats.df)
      #
      #RandNObsSubsets <- data.frame(GLMRandNObsFiltSubsetEvalStats.df[,1], stringsAsFactors=FALSE)
      #colnames(RandNObsSubsets) <- "VarNames"
      #################
      ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
        ...
        OutDirect2Old <- paste0(OutDirectpsaOld, "/", output2)
        setwd(OutDirect2Old)
        MaxEntOldEvalStats.df <- data.frame(read.csv(paste0(Species, "MaxentResults_TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_Random", FullNObs, "_", FullNObs+Round2(PlusNum/2,0), ".csv"), stringsAsFactors=FALSE))
        RandNObsSubsets <- MaxEntOldEvalStats.df[1:(nrow(MaxEntOldEvalStats.df)/2),2, drop=FALSE]
        tail(RandNObsSubsets)
        setwd(OutDirect2)
      ###########################
      # Use difference between evaluation statistics between train and test data to evaluate overfitting
      ###############################################################################
      ...
      for (j in 1:length(DataSetTypes)) {
        ...
        selectedRows <- (GLMWrapperTrainTestSubsetEvalStats.df$VarNames %in% GLMTestSubsetEvalStats.df$VarNames)
        GLMTestSubsetEvalStats.df2 <- GLMWrapperTrainTestSubsetEvalStats.df[selectedRows,]
        ## Take apart by DataType, reorder each by rank and reassemble
        GLMTrainSubsetEvalStats.df3 <- GLMTestSubsetEvalStats.df2[which(GLMTestSubsetEvalStats.df2$DataType=="WrapperTrain"),]
        GLMTrainSubsetEvalStats.df4 <- GLMTrainSubsetEvalStats.df3[match(unlist(GLMTestSubsetEvalStats.df$VarNames), GLMTrainSubsetEvalStats.df3$VarNames), ]
        #
        GLMTestSubsetEvalStats.df3 <- GLMTestSubsetEvalStats.df2[which(GLMTestSubsetEvalStats.df2$DataType=="WrapperTest"),]
        GLMTestSubsetEvalStats.df4 <- GLMTestSubsetEvalStats.df3[match(unlist(GLMTestSubsetEvalStats.df$VarNames), GLMTestSubsetEvalStats.df3$VarNames), ]
        #
        GLMDiffTestTrainSubsetEvalStats.df3 <- GLMTestSubsetEvalStats.df2[which(GLMTestSubsetEvalStats.df2$DataType=="WrapperDiffTestTrain"),]
        GLMDiffTestTrainSubsetEvalStats.df4 <- GLMDiffTestTrainSubsetEvalStats.df3[match(unlist(GLMTestSubsetEvalStats.df$VarNames), GLMDiffTestTrainSubsetEvalStats.df3$VarNames), ]
        # Reassemble
        GLMSubsetEvalStats.df <- rbind(GLMTrainSubsetEvalStats.df4, GLMTestSubsetEvalStats.df4, GLMDiffTestTrainSubsetEvalStats.df4)
        GLMSubsetEvalStats.df$DataType <- paste0(DataSetType, GLMSubsetEvalStats.df$DataType)
        GLMSubsetEvalStats.df$SetName <- SetName
        GLMSubsetEvalStats.df$Set <- DataSetType
        GLMSubsetEvalStats.df$TotSubsets <- ChosenModSets
        GLMSubsetEvalStats.df$PresTrainPoints <- nrow(PresenceDat.df[kfoldgrpp==1,])
        GLMSubsetEvalStats.df$Rep <- Rep
        ncol(GLMSubsetEvalStats.df)
        head(GLMSubsetEvalStats.df)
        GLMSubsetEvalStats.df <- GLMSubsetEvalStats.df[, colnames(GLMSubsetEvalStats.df)[c(1:4,17:21,5:16)]]
        #
        GLMKeepEvalStatsL1[[j]] <- GLMSubsetEvalStats.df
      }
      ###############
      setwd(OutDirect2)
      #SubsetRuns <- list(TopNObsSubsets[1:NObs,], TopNObsSubsets[(NObs+1):(NObs*2),], TopNObsSubsets[(NObs*2+1):(NObs*3),], TopNObsSubsets[(NObs*3+1):(NObs*4),],RandNObsSubsets[1:NObs,], RandNObsSubsets[(NObs+1):(NObs*2),], RandNObsSubsets[(NObs*2+1):(NObs*3),])
      ...
      for (j in 1:length(DataSetTypes)) {
        ...
        DataSetType <- DataSetTypes[j]
        ## Evaluate random subsets for NumberModSets number of subsets
        system.time(GLMTrainTestSubsetEvalStats.df1 <- GLMBinomialMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
        # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
        GLMTrainTestSubsetEvalStats.df <- subset(GLMTrainTestSubsetEvalStats.df1, AICc2!="Inf")
        nrow(GLMTrainTestSubsetEvalStats.df)
         ## Separate out training and testing data
        GLMTrainSubsetEvalStats.df <- GLMTrainTestSubsetEvalStats.df[which(GLMTrainTestSubsetEvalStats.df$DataType=="FinalTrain"),]
        GLMTrainSubsetEvalStats.df$DataType <- paste0(DataSetType, "FinalTrain")
        GLMTestSubsetEvalStats.df <- GLMTrainTestSubsetEvalStats.df[which(GLMTrainTestSubsetEvalStats.df$DataType=="FinalTest"),]
        ...
        GLMSubsetEvalStats.df$Rep <- Rep
        ncol(GLMSubsetEvalStats.df)
        GLMSubsetEvalStats.df <- GLMSubsetEvalStats.df[, colnames(GLMSubsetEvalStats.df)[c(1:4,13:21,5:12)]]
        head(GLMSubsetEvalStats.df)
        #
        GLMKeepEvalStatsL2[[j]] <- GLMSubsetEvalStats.df
      }
      ###############
      GLMKeepEvalStats.df1 <- do.call(rbind, GLMKeepEvalStatsL1)
      nrow(GLMKeepEvalStats.df1)
      GLMKeepEvalStats.df2 <- do.call(rbind, GLMKeepEvalStatsL2)
      nrow(GLMKeepEvalStats.df2)
      GLMKeepEvalStats.df3 <- rbind(GLMKeepEvalStats.df1, GLMKeepEvalStats.df2)
      nrow(GLMKeepEvalStats.df3)
      GLMKeepEvalAllStatsL[[OutCount]] <- GLMKeepEvalStats.df3
      ### Summarize Output
      ###Calculate Mean and Standard Deviation Values
      setwd(OutDirectpsa)
      ...
      ncol(GLMKeepEvalStatsAllSets.df)
      ## Save data
      write.table(GLMKeepEvalStatsAllSets.df, file=paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_", "SummaryStats_of", FinalModelVariables, "of", TotVars, "Vars", FSAType,"_", RunType, ".csv"), sep=",", col.names=NA)
      ### Summarize and save raw data as loop progresses
      GLMKeepEvalStats.df <- do.call(rbind, GLMKeepEvalAllStatsL)
      # Save data
      write.table(GLMKeepEvalStats.df , file=paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_TrainVsTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff", ".csv"), sep=",", col.names=NA)
      #
     }
  }
}
...
setwd(OutDirectpsa)
# Read data back in and sort
GLMKeepEvalStats.df <- data.frame(read.csv(paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_TrainVsTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
nrow(GLMKeepEvalStats.df)
ncol(GLMKeepEvalStats.df)
head(GLMKeepEvalStats.df)
tail(GLMKeepEvalStats.df)
#
#EvaluationStats <- GLMKeepEvalStats.df1
...
##################################################################################
### Calculate adjusted AICc_bg (AICc_bg_adj) for comparison with MaxEnt. For each
### SubsetVariableNumber, find the equation for regression of AICc_bg versus AICc2.
### Then double the y-intercept value and add it to AICc_bg
#################################################################################
##
##################################################################################
### Calculate adjusted AICc_bg (AICc_bg_adj) for comparison with MaxEnt. For each
...
################################################################################
### Calculate AICc_bg_reg save to data set
## Read in the regression relationship between AICc_bg and AICc from the AUC selection run
OutDirectpsaGLMAUC <- gsub("GLMBinomial", "GLMBinomial", OutDirectpsa)
setwd(OutDirectpsaGLMAUC)
fitAICStats.df <- data.frame(read.csv(paste0(Species, "_AllWrapperTest_AICc_bg_Vs_AICc2_RegressionStats.csv"), stringsAsFactors=FALSE))
head(fitAICStats.df)
# Read in the regression model
fitAIC <- readRDS("LinearAICc2toAICc_bgModel.rds")
## Calculate AICc_bg_reg using regression equation
xDatIn <- data.frame(AICc2 = GLMKeepEvalStats.df$AICc2)
head(xDatIn)
GLMKeepEvalStats.df$AICc_bg_reg <- predict(fitAIC, newdata=xDatIn)
...
## Save revised data
setwd(OutDirectpsa)
write.table(GLMKeepEvalStats.df, file=paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_TrainVsTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff2.csv"), sep=",", row.names=FALSE)

#######################################################################################
###Calculate Mean and Standard Deviation Values including AICcbg_reg
setwd(OutDirectpsa)
RunType <- "Final"
EvaluationStats <- GLMKeepEvalStats.df
ncol(EvaluationStats)
EvaluationStats$Model <- ModelName
EvaluationStats <- EvaluationStats[, colnames(EvaluationStats)[c(23,1:22)]]
head(EvaluationStats)
#str(EvaluationStats)
#FSAType <- RankStatistic
OutName <- paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_SubsetSummaryStats_", SubsetVariableNumber, "of", TotVars, "Vars", FSAType,"_", RunType, "2.csv")
SortGroups <- c("Model", "Rep", "DataType", "Set", "SetName", "SubsetVariableNumber", "TotSubsets")
## All statistics, and only statistics, should be at and after the column specified below
StatVarFirstColumn <- 9
OutDirectIn <- OutDirectpsa
...
head(EvaluationStatsSummary.df)
# Read in data and resort by DataType
GLMKeepEvalStatsNewSummary.df <- data.frame(read.csv(paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_SubsetSummaryStats_", SubsetVariableNumber, "of", TotVars, "Vars", FSAType,"_", RunType, "2.csv"), stringsAsFactors=FALSE))
head(GLMKeepEvalStatsNewSummary.df)
GLMKeepEvalStatsNewSummary.dfs <- arrange(GLMKeepEvalStatsNewSummary.df, DataType)
# Drop first column of row names
GLMKeepEvalStatsNewSummary.dfs <- GLMKeepEvalStatsNewSummary.dfs[,c(-1)]
GLMKeepEvalStatsNewSummary.dfs[1:20,]
write.table(GLMKeepEvalStatsNewSummary.dfs, file=paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_SubsetSummaryStats_", SubsetVariableNumber, "of", TotVars, "Vars", FSAType,"_", RunType, "2.csv"), sep=",", col.names=NA)
...
#
# Keep only DataTypes including string "FinalTest"
GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df[grep("FinalTest", GLMKeepEvalStats.df$DataType), ]
GLMKeepEvalStats.df2[1:50,]
head(GLMKeepEvalStats.df2)
tail(GLMKeepEvalStats.df2)
###Calculate Mean and Standard Deviation Values
RunType <- "Final"
EvaluationStats <- GLMKeepEvalStats.df2
OutName <- paste0(Species, RankStatistic, "GLM_", NumberModSets, "Setsof_", SubsetVariableNumber, "_", FullNObs, "N_StatSummary.csv")
...
head(EvalStatsOuts)
write.table(EvalStatsOuts, file=paste0(Species, RankStatistic, "GLM_", SetName, "_", FullNObs, "Stat", SubsetVariableNumber, "VariableRunsby", NumberModSets, "SetsSummaryTable.csv"), sep=",", col.names=NA)
##
#
#############################################
...
SetNameF <- "AllVariables"
StatTypeNames <- c("AUC", "AICc2", "AICc_bg_reg", "AUCdiff")
...
for(Rep in RepList) {
  ...
  #Rep = 1
  RepNum <- Rep
  GLMKeepEvalStats.df3 <- subset(GLMKeepEvalStats.df2, Rep==RepNum)
  head(GLMKeepEvalStats.df3)
  for(StatType in StatTypeNames) {
    ...
    for(RankType in RankTypeNames) {
      ...
      for(RandomType in RandomTypeNames) {
        ...
        GLMTestStats <- subset(GLMKeepEvalStats.df3, Set %in% c(RankType, RandomType))
        Count <- Count + 1
        if(StatType==StatTypeNames[1]) {
          # Identify set as Top or Random
          GLMTestStats$SetType <- "Top"
          ncol(GLMTestStats)
          head(GLMTestStats)
          GLMTestStats <- GLMTestStats[,c(1:9, 21, 10:20)]
          GLMTestStats <- within(GLMTestStats, SetType[grepl("Rand", DataType)] <- 'Random')
          # Perform summary stats for excel plotting
          ###Calculate Mean and Standard Deviation Values
          RunType <- "Final"
          EvaluationStats <- GLMTestStats
          ...
          head(EvalStatsOuts)
          #
          GLMEvalStatsL[[Count]] <- EvalStatsOuts
        }
        #
        setwd(OutDirectpsa)
        ## Use Welch correction t test
        RankWelchttest <- t.test(as.formula(paste0(StatType, " ~ DataType")), data=GLMTestStats)
        #str(RankWelchttest)
      ...
      if(StatType==StatTypeNames[1]) {
        GLMEvalStatsAll.df <- do.call(rbind, GLMEvalStatsL)
        ## Sort data
        GLMEvalStatsAll.dfs <- arrange(GLMEvalStatsAll.df, SetType, Statistic, SetType)
        write.table(GLMEvalStatsAll.dfs, file=paste0(Species, "WelchttestStatsby", FullNObs, "from", NumberModSets, SetNameF, "Setsof", SubsetVariableNumber, "Vars", RankType, "vsRandom_DataTable", "_Rep", Rep, ".csv"), sep=",", col.names=NA)
    }
    }
  }
}
...
### to generate GLM projection maps and rank by regional variables to choose four top selected
...
GLMKeepEvalStats.df1 <- data.frame(read.csv(paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_TrainVsTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
nrow(GLMKeepEvalStats.df1)
ncol(GLMKeepEvalStats.df1)
head(GLMKeepEvalStats.df1)
tail(GLMKeepEvalStats.df1)
...
GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df1[grep(paste0("FinalTest"), GLMKeepEvalStats.df1$DataType), ]
nrow(GLMKeepEvalStats.df2)
head(GLMKeepEvalStats.df2)
tail(GLMKeepEvalStats.df2)
...
for(Rep in RepList) {
  ...
  for(ModelType in ModelTypeNames) {
    ...
    GLMTestKeepEvalStats.df3 <- GLMKeepEvalStats.df2[which(GLMKeepEvalStats.df2$Rep==Rep & GLMKeepEvalStats.df2$DataType==paste0(ModelType, "FinalTest")),]
    # Keep only first NumProjections/3 number of rows
    Count <- Count + 1
    if(NumProjections > 6) {
      GLMKeepEvalStatsL[[Count]] <- GLMTestKeepEvalStats.df3[1:(NumProjections/3),]
    } else {
      GLMKeepEvalStatsL[[Count]] <- GLMTestKeepEvalStats.df3[1:2, ]
    }
  }
}
#############
GLMKeepEvalStats.df4 <- do.call(rbind, GLMKeepEvalStatsL)
## Sort data by DataType
GLMKeepEvalStats.df4s <- arrange(GLMKeepEvalStats.df4, DataType)
nrow(GLMKeepEvalStats.df4s)
## Save data
setwd(OutDirectpsa)
write.table(GLMKeepEvalStats.df4s, file=paste0(Species, ModelName, FullNObs, "_", NumberModSets, "TopSetsFinalTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_.csv"), sep=",", col.names=NA)
###
...
for(ModelType in ModelTypeNames) {
  #ModelType <- paste0("TopAUC",FullNObs)
  ModelType <- paste0("Random",FullNObs)
  # Keep only DataTypes including string of ModelType for Final Test
  GLMKeepEvalStats.df5 <- GLMKeepEvalStats.df4s[grep(paste0(ModelType, "FinalTest"), GLMKeepEvalStats.df4s$DataType), ]
  #
  if(NumProjections < 6) {
    # Sort by RSFSA Wrapper
    GLMKeepEvalStats.df5 <- arrange(GLMKeepEvalStats.df5, -GLMKeepEvalStats.df5[[RankStatistic]])
  }
  #
...
#    write.table(data.frame(NObskfoldgrpa), file=paste0(Species, PsAbsBuffs, "PseudoabsenceDat", NObsJ, "Kfold", ".csv"), sep=",")
    ...
    #################
    VariableSubset <- data.frame(as.character(GLMKeepEvalStats.df5[k,1]), stringsAsFactors=FALSE)
    colnames(VariableSubset) <- "VarNames"
    # Read in a variable subset for testing
    setwd(InDirect)
    VariableSubsetsIn <- read.csv(paste0(SpecShort, "RandomGLM250_", SubsetVariableNumber, "VariableSets.csv"), stringsAsFactors=FALSE, na.strings = c("NA", ""))
    VariableSubsetsIn <- VariableSubsetsIn[,2, drop=FALSE]
    head(VariableSubsetsIn)
    VariableSubset <- VariableSubsetsIn[1, , drop = FALSE]
    ####
    VarNames <- unlist(strsplit(as.character(VariableSubset[,1]),"-"))
    SubsetVariableNumber <- length(VarNames)
    Rep <- GLMKeepEvalStats.df5[k,8]
    #Rep=1
    #
    ...
    ######################
    #### FIRST: Find the threshold values for the GLM model using a training grid
    #### model
    ...
    ##
    setwd(OutDirectpsa)
    output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", Loop)
    #output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", Loop, "Minus", 14)
  ...
  #  names(Predictors) <- gsub("_NS", "", names(Predictors))
    ...
    CRS.In <- CRS.WGS84
    #
    Time <- system.time(GLMKeepEvalStats.df <- GLMBinomialSubset.GridTrainTestEvalAIC_AUCpa_Calib(Species, VariableNamesIn, PredictorsIn, SubsetVariableNumber, TotPres, Run, SetName, VariableSubset, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrppin, kfoldgrpain, CRS.In, OutDirectIn, Output=TRUE))
    #
    head(GLMKeepEvalStats.df)
    # Save results
    setwd(OutDirectIn)
    write.table(GLMKeepEvalStats.df , file=paste0(Species, "GLM", DataSet, "_TrainVsTestStatsThreshCal_of", NumberModSets, "_", SetName, SubsetVariableNumber, "_", Loop, ".csv"), sep=",", col.names=NA)
    #
    #GLMMod <- readRDS("GLMModel.rds") # Reads in stored GLM model from above function
    Count <- Count + 1
    ## Add ModelType to output
    GLMKeepEvalStats.df$ModelType <- ModelType
    ncol(GLMKeepEvalStats.df)
    # Re-order columns
    GLMKeepEvalStats.df <- GLMKeepEvalStats.df[,c(1:3, 17, 3:16)]
    # Replace Run with Rep
    GLMKeepEvalStats.df$Rep <- Rep
    ncol(GLMKeepEvalStats.df)
    GLMKeepEvalStats.df <- GLMKeepEvalStats.df[,c(1:7, 18, 8:17)]
    GLMKeepEvalStatsL[[Count]] <-  GLMKeepEvalStats.df
  }
}
t2 <- Sys.time()
#############################################################################
difftime(t2,t1, units = "mins")
###
GLMKeepEvalStatsAll.df <- do.call(rbind, GLMKeepEvalStatsL)
## Sort by DataType
GLMKeepEvalStatsAll.dfs  <- arrange(GLMKeepEvalStatsAll.df, DataType)
## Save data
setwd(OutDirectpsa)
write.table(GLMKeepEvalStatsAll.dfs, file=paste0(Species, "GLM", DataSet, "_TrainVsTestStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), sep=",", col.names=NA)
...
#NumberModSets <- 10000
setwd(OutDirectpsa)
GLMKeepEvalGridStats.df <- data.frame(read.csv(paste0(Species, "GLM", DataSet, "_TrainVsTestStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
##
## Keep only DataType of Test
GLMKeepEvalGridStats.df1 <- subset(GLMKeepEvalGridStats.df, DataType=="Test")
# Test for significant linear regression between AICc and AICcbg
AICc_AICcbg.lm <- lm(AICc ~ AICc_bg, data=GLMKeepEvalGridStats.df1)
...
####################################################################################
## Read in above produced csv files from python and calculate percent overlaps of GLM models
## with breeding range for portions of GLM model both inside and outside breeding range
##
FinalModelVariables <- 15
NumProjections <- 30
NObsJ <- 3
...
SubsetVariableNumber <- FinalModelVariables
setwd(OutDirectpsa)
GLMKeepEvalStats.df1 <- data.frame(read.csv(paste0(Species, ModelName, FullNObs, "_", NumberModSets, "TopSetsFinalTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
tail(GLMKeepEvalStats.df1)
nrow(GLMKeepEvalStats.df1)
#
####################################
...
## Read in GLM model csv cell counts from directory where grids stored
#ModelTypeNames <- c(paste0("TopAUC", FullNObs), paste0("Random", FullNObs))
ModelTypeNames <- c(paste0("TopAUC",FullNObs))
GLMKeepEvalStatsL <- list()
##############################################################################
...
for(ModelType in ModelTypeNames) {
  #ModelType <- paste0("TopAUC", FullNObs)
  # Keep only DataTypes including string of ModelType for Final Test
  GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df1[grep(paste0(ModelType, "FinalTest"), GLMKeepEvalStats.df1$DataType), ]
  nrow(GLMKeepEvalStats.df2)
  head(GLMKeepEvalStats.df2)
  ##
  for(k in 1:NumProjections) {
  #for(k in 1) {
    ...
    GLMKeepEvalStats.df3 <- GLMKeepEvalStats.df2[Loop,]
    ##########
    setwd(OutDirectpsa)
    output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, FinalModelVariables, "_ThresholdCalib", Loop)
    OutDirectSub <- paste0(OutDirectpsa, "/", output3)
    ##
    setwd(OutDirectSub)
    ## Read in csv files of GLM grid cell counts inside breeding area
    GLMProjBreedRangeArea.df <- data.frame(read.csv(paste0(Species, Loop, "mxt", FinalModelVariables, "apresareatable.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
    ## Calculate Megahectares area overlapping breeding range
    BreedOverlapArea <- (GLMProjBreedRangeArea.df$AREA * 0.0001)/ 1000000 # convert from sq. meters to hectares then megehectares
    ## Calculate percent of breeding range occupied by GLM projection, Regional Fraction Index (RFI)
    RegionalFractionIndex <- BreedOverlapArea/BreedingRangeArea
    ## Read in csv files of GLM grid cell counts outside breeding area
    GLMProjNonBreedRangeArea.df <- data.frame(read.csv(paste0(Species, Loop, "mxt", FinalModelVariables, "anpresareatable.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
    ## Calculate Megahectares area overlapping breeding range
    NonBreedArea <- (GLMProjNonBreedRangeArea.df$AREA * 0.0001)/ 1000000 # convert from sq. meters to hectares then megehectares
    ## Calculate percent of breeding range represented by GLM projection outside of breeding range in Background Evaluation Extent (BEE), Regional Excess Index (REI)
    ...
    GLMKeepEvalStats.df3$BRA <- BreedingRangeArea
    ## Then add RFI, REI, and RCI
    GLMKeepEvalStats.df3$RFI <- RegionalFractionIndex
    GLMKeepEvalStats.df3$REI <- RegionalExcessIndex
    GLMKeepEvalStats.df3$RBI <- RegionalBoundingIndex
    # Record model number/rank from original Wrapper ranking
    GLMKeepEvalStats.df3$ModelNumber <- Loop
    ncol(GLMKeepEvalStats.df3)
    GLMKeepEvalStats.df3 <- GLMKeepEvalStats.df3[,c(1:5, 22, 6:21)]
    ## Save output to list
    Count <- Count + 1
    GLMKeepEvalStatsL[[Count]] <- GLMKeepEvalStats.df3
  }
}
t2 <- Sys.time()
#############################################################################
...
REIrank <- 1/3
RCIrank <- 1/3
decision.mat <- as.matrix(GLMKeepEvalStatsAll.df[,c(20:22)])
head(decision.mat)
# Normalize AICc_bg and AUC from zero to 1
normalizef <- function(x) c((x+(max(x)-min(x))-max(x))/(max(x)-min(x)))
decision.matn <- as.matrix(apply(decision.mat,2,normalizef))
...
head(MMOORA.Rank)
# Join MMOORA rank to original data
GLMKeepEvalStatsAll.df$MMOORA_Rank <- MMOORA.Rank[,8]
head(GLMKeepEvalStatsAll.df)
# Sort data by MMOORA rank
GLMKeepEvalStatsAll.dfs <- arrange(GLMKeepEvalStatsAll.df, MMOORA_Rank)
## Save data
setwd(OutDirectpsa)
write.table(GLMKeepEvalStatsAll.dfs, file=paste0(Species, "GLM", DataSet, "_TrainVsTestandAreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), sep=",", col.names=NA)
...
setwd(OutDirectpsa)
GLMKeepEvalAreaStatsAll.dfs <- data.frame(read.csv(paste0(Species, "GLM", DataSet, "_TrainVsTestandAreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
# Keep only rankings for model type
GLMKeepEvalAreaStatsAll.df2 <- GLMKeepEvalAreaStatsAll.dfs[which(GLMKeepEvalAreaStatsAll.dfs$DataType==paste0("TopAUC", FullNObs, "FinalTest")),]
FinalModelRowNumberList.df <- as.data.frame(GLMKeepEvalAreaStatsAll.df2$ModelNumber)
...
TopSelSubsets <- as.data.frame(GLMKeepEvalAreaStatsAll.df2[1:FinalNumberModels,1], stringsAsFactors=FALSE)
...
for(p in 1:NumProjections) {
  #p=1
  setwd(OutDirectpsa)
  output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", p)
  #output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", Loop, "Minus", 14)
  OutDirectSub <- paste0(OutDirectpsa, "/", output3)
  setwd(OutDirectSub)
  GLMResults.df <- data.frame(read.csv("GLMResults.csv", header = TRUE, sep=','), stringsAsFactors=FALSE)
  PermImp.df <- GLMResults.df[,(7+SubsetVariableNumber+1):(6+(2*SubsetVariableNumber)+1)]
...
#################
#####################################################################################################
### Run GLM 3-fold cross-valication evaluation mode with jacknife option to assess variable importance with all variables
### for top model
#####################################################################################################
...
for(NumberModel in 1:NumberTopModels) {
  ...
  GLMPresTrainData <- PresenceDat.df
  head(GLMPresTrainData)
  GLMAbsTrainData <- BackgroundDat.df
  head(GLMAbsTrainData)
  TrainSWD <- rbind(GLMPresTrainData[,4:ncol(GLMPresTrainData)], GLMAbsTrainData[,4:ncol(GLMAbsTrainData)])
  head(TrainSWD)
  TrainPresID <- data.frame(rep(1,nrow(GLMPresTrainData)))
  colnames(TrainPresID) <- "ID"
  TrainAbsID <- data.frame(rep(0,nrow(GLMAbsTrainData)))
  ...
  head(TrainSWD)
  ######
  # Create subdirectory for GLM output
  output3 <- paste0("GLMEvaluationMode_FinalSubset_", SetNameF, SubsetVariableNumber, "_Set", TopModel)
  dir.create(paste0(OutDirectpsa, "/", output3))
  OutDirectSub <- paste0(OutDirectpsa, "/", output3)
  setwd(OutDirectSub)
  #
  #system.file("java", package="dismo")
  ## Check if have categorical variable to add to GLM arguments
  if(exists("CatVarsPrefix")==TRUE) {
    GLMCatArg <- paste0("togglelayertype=", CatVarsPrefix)
    GLMEvalArgs <- c(GLMEvalArgs1, GLMCatArg)
    } else {
    GLMEvalArgs <- GLMEvalArgs1
  }
  ...
  system.time(GLMOut <- GLM(TrainSWD, TrainPresAbsID, args=GLMEvalArgs, path=OutDirectSub))
}
t2 <- Sys.time()
###################################################################################
difftime(t2,t1, units = "mins")
...
for(FutClimSubDir in FutClimSubDirList) {
  ...
  for(ModelType in ModelTypeNames) {
    ...
    # Read in data with rankings
    setwd(OutDirectpsa)
    GLMKeepEvalAreaStatsAll.df <- data.frame(read.csv(paste0(Species, "GLM", DataSet, "_TrainVsTestandAreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
    # Keep only rankings for model type
    GLMKeepEvalAreaStatsAll.df2 <- GLMKeepEvalAreaStatsAll.df[which(GLMKeepEvalAreaStatsAll.df$DataType==paste0(ModelType, "FinalTest")),]
    FinalModelRowNumberList.df <- as.data.frame(GLMKeepEvalAreaStatsAll.df2$ModelNumber)
    ...
    for(FinalModelRowNumber in FinalModelRowNumberList) {
      ...
      ## Read in saved calibration threshold and variables of final chosen GLM model
      setwd(OutDirectpsa)
      output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", FinalModelRowNumber)
      #SubsetVariableNumber <- 14
      #output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", Loop, "Minus", 14)
      OutDirectSub <- paste0(OutDirectpsa, "/", output3)
      setwd(OutDirectSub)
      ## Read in calibration threshold
      GLMKeepEvalStats.df <- data.frame(read.csv(paste0(Species, "GLM", DataSet, "_TrainVsTestStatsThreshCal_of", NumberModSets, "_", SetName, SubsetVariableNumber, "_", FinalModelRowNumber, ".csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
      #
      SubsetTestMeans <- GLMKeepEvalStats.df[which(GLMKeepEvalStats.df$DataType=="Test"),]
      ...
      ## Read in saved GLM model
      GLMMod <- readRDS("GLMModel.rds")
      #### Subset Predictors by Variables
      PredictorsFut <- subset(PredictorsAll, unlist(VarNames))
      ################
      ## Specify output directories
      output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", FinalModelRowNumber, "_", FutClimSubDir)
      #output3 <- paste0("GLMProj_", ModelType, "_Subsetof", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_ThresholdCalib", FinalModelRowNumber, "_Minus14_", FutClimSubDir)
      dir.create(paste0(OutDirectpsa, "/", output3))
      OutDirectSub <- paste0(OutDirectpsa, "/", output3)
      # Generate grid model projection for future climate grids
      system.time(GLM.score1 <- predict(GLMMod, PredictorsFut))
      #plot(GLM.score1)
      # Multiply raw grid by 1000 and convert to integer
      GLM.score <- calc(GLM.score1, function(x) as.integer(x * 1000) )
      #plot(GLM.score)
      # Save for Arc as a geoTIFF grid
      OutGridID <- paste0(ModelType, "_", FinalModelRowNumber, "ThreshCal_", SetNameF, "_", FutClimSubDir)
      setwd(OutDirectSub)
      writeRaster(GLM.score, paste0(Species, "GLM", OutGridID, "_", SubsetVariableNumber, "Vars_Beta", BetaMult), format = "GTiff", overwrite=TRUE)
      #
      ################################################################################
      ### Calibrate model using using above calculated ThresholdK at maximum TSS
      ### for current climate model
      GLM.scorecal <- calc(GLM.score, function(x) ifelse(x < ThresholdK, 0, 1) )
      #plot(GLM.scorecal)
      writeRaster(GLM.scorecal, paste0(Species, "GLM", OutGridID, "_", SubsetVariableNumber, "Vars_Beta", BetaMult, "Cal"), format = "GTiff", overwrite=TRUE)
      }
    }
}
t2 <- Sys.time()
...
for(ModelType in ModelTypeNames) {
  ...
  GLMKeepEvalStats.df2 <- GLMKeepEvalStats.df1[which(GLMKeepEvalStats.df1$SubsetVariableNumber==FinalModelVariables & GLMKeepEvalStats.df1$DataType==paste0(ModelType, "FinalTest")),]
  ####
  for(FinalModelRowNumber in FinalModelRowNumberList) {
    #FinalModelRowNumber=17
    for(FutClimSubDir in FutClimSubDirList) {
      #FutClimSubDir <- FutClimSubDirList[[1]]
      SelectedSubsetData <- GLMKeepEvalAreaStatsAll.df2[which(GLMKeepEvalAreaStatsAll.df2$ModelNumber==FinalModelRowNumber),]
      ## Subset original evaluation data using VarNames
      selectedRows <- (GLMKeepEvalStats.df2$VarNames %in% SelectedSubsetData$VarNames)
      GLMTestSubsetEvalStats.df <- GLMKeepEvalStats.df2[selectedRows,]
      #
      TopModelSet.df <- GLMTestSubsetEvalStats.df[,1:8]
      ...
      setwd(OutDirectSub)
      ## Read in csv files of GLM grid cell counts inside breeding area
      GLMProjBreedRangeArea.df <- data.frame(read.csv(paste0(SpecShort, FutClimSubDir, "presareatable.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
      ## Calculate Megahectares area overlapping breeding range
      BreedOverlapArea <- (GLMProjBreedRangeArea.df$AREA * 0.0001)/ 1000000 # convert from sq. meters to hectares then megehectares
      ## Calculate percent of breeding range occupied by GLM projection, Regional Fraction Index (RFI)
      RegionalFractionIndex <- BreedOverlapArea/BreedingRangeArea
      ## Read in csv files of GLM grid cell counts outside breeding area
      GLMProjNonBreedRangeArea.df <- data.frame(read.csv(paste0(SpecShort, FutClimSubDir, "npresareatable.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
      ## Calculate Megahectares area overlapping breeding range
      NonBreedArea <- (GLMProjNonBreedRangeArea.df$AREA * 0.0001)/ 1000000 # convert from sq. meters to hectares then megehectares
      ## Calculate percent of breeding range represented by GLM projection outside of breeding range in Background Evaluation Extent (BEE), Regional Excess Index (REI)
      RegionalExcessIndex <- NonBreedArea/BreedingRangeArea
      ...
      ## Add Breeding Range Area in Megahectares
      GLMKeepEvalStats.df3 <- TopModelSet.df
      GLMKeepEvalStats.df3$DataType <- ModelType
      GLMKeepEvalStats.df3$BRA <- BreedingRangeArea
      ...
      GLMKeepEvalStats.df3$RBI <- RegionalBoundingIndex
      # Record model number/rank from original Wrapper ranking
      GLMKeepEvalStats.df3$FutClimate <- FutClimSubDir
      GLMKeepEvalStats.df3$ModelNumber <- FinalModelRowNumber
      ncol(GLMKeepEvalStats.df3)
      GLMKeepEvalStats.df3 <- GLMKeepEvalStats.df3[,c(1, 13, 2:8, 14, 9:12)]
      ## Save output to list
      Count <- Count + 1
      GLMKeepEvalStatsL[[Count]] <- GLMKeepEvalStats.df3
    }
...
## Save data
GLMKeepEvalStatsL[[1]]
GLMKeepEvalStatsFuture.df <- do.call(rbind, GLMKeepEvalStatsL)
ncol(GLMKeepEvalStatsFuture.df)
nrow(GLMKeepEvalStatsFuture.df)
head(GLMKeepEvalStatsFuture.df)
#
setwd(OutDirectpsa)
write.table(GLMKeepEvalStatsFuture.df, file=paste0(Species, "GLMFutureClimate", "_AreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", "_TopVsRandom", NumProjections, "Sets.csv"), sep=",", col.names=NA)
###################################################################################
...
GLMAreaStatsCurrent.df <- data.frame(read.csv(paste0(Species, "GLM", DataSet, "_TrainVsTestandAreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
# Reformat to match future climate data
CurrentRegionalIndices.df <- GLMAreaStatsCurrent.df[,c(1:5,7:9,6,19:22)]
...
GLMAreaStatsFuture.df <- data.frame(read.csv(paste0(Species, "GLMFutureClimate", "_AreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", "_TopVsRandom", NumProjections, "Sets.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
colnames(GLMAreaStatsFuture.df)[2] <- "Climate"
head(GLMAreaStatsFuture.df)
ncol(GLMAreaStatsFuture.df)
# Combine current and future regional indices together
GLMAreaStatsClimate.df <- rbind(CurrentRegionalIndices.df, GLMAreaStatsFuture.df)
GLMAreaStatsClimate.dfs <- arrange(GLMAreaStatsClimate.df, Climate, DataType, ModelNumber, Climate)
# Create ClimateDataType combining Climate and Set
GLMAreaStatsClimate.dfs$ClimDataType <- paste0(GLMAreaStatsClimate.dfs$Climate, "_", GLMAreaStatsClimate.dfs$Set)
# Rearrange columns
GLMAreaStatsClimate.dfs <- GLMAreaStatsClimate.dfs[,c(1:3,15,5:14)]
head(GLMAreaStatsClimate.dfs)
ncol(GLMAreaStatsClimate.dfs)
## Save combined data
write.table(GLMAreaStatsClimate.dfs, file=paste0(Species, "GLMCurrentFutureClimate", "_AreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", "_TopVsRandom", NumProjections, "Sets.csv"), sep=",", col.names=NA)
###
### Calculate Mean and Standard Deviation Values
EvaluationStats <- GLMAreaStatsClimate.dfs
OutName <- paste0(Species, "GLMProjection_Top3CurrentFutureModelAreaStats_", SubsetVariableNumber, "Vars_", "_Summary.csv")
...
EvalStatsOuts <- arrange(EvalStatsOut, Statistic)
head(EvalStatsOuts)
write.table(EvalStatsOuts, file=paste0(Species, "GLMProjection_Top3CurrentFutureModelAreaStats_", SubsetVariableNumber, "Vars_", "_SummaryTable.csv"), sep=",", col.names=NA)
...
for(StatType in StatTypeNames) {
  ...
  for(ModelType in ModelTypeNames) {
    ...
    GLMTestStats1 <- GLMAreaStatsClimate.dfs[which(GLMAreaStatsClimate.dfs$Set==ModelType),]
    ## Use Welch correction one way ANOVA for unequal variances for primary test among all possible climate comparisons
    WelchANOVA <- oneway.test(as.formula(paste0(StatType, " ~ ClimDataType")), data=GLMTestStats1, na.action=na.omit, var.equal=FALSE)
    ...
    for(ClimateType in ClimateTypeNames) {
      ...
      GLMTestStats <- GLMTestStats1[which(GLMTestStats1$Climate=="Current" | GLMTestStats1$Climate==ClimateType),]
      N <- nrow(GLMTestStats)/2
      ## Use Welch correction t test
      RankWelchttest <- t.test(as.formula(paste0(StatType, " ~ Climate")), data=GLMTestStats)
      #str(RankWelchttest)
...
##
#####################################################################################
### First, run through the series of python scripts below to reproject GLM model
...
GLMKeepEvalAreaStatsAll.df3 <- GLMKeepEvalAreaStatsAll.df2[1:4,]
ncol(GLMKeepEvalAreaStatsAll.df3)
...
for(FinalModelRowNumber in FinalModelRowNumberList) {
  ...
  for(EcoRegNum in 1:53) {
    #EcoRegNum<-7
    GLMKeepEvalAreaStatsAll.df4 <- GLMKeepEvalAreaStatsAll.df3[which(GLMKeepEvalAreaStatsAll.df3$ModelNumber==FinalModelRowNumber),]
    ...
    setwd(OutDirectSub)
    #########################
    ## Read in csv files of GLM grid cell counts inside projection area
    ...
    #
    ## Calculate percent of ecoregion occupied by GLM current projection
    ...
    ## Add data to evaluation statistics
    ## Add Breeding Range Area in Megahectares
    GLMKeepEvalAreaStatsAll.df4$BEEENMArea <- BEEEcoregionProjPresArea
    GLMKeepEvalAreaStatsAll.df4$BEEPercENMArea <- BEEPercEcoregion
    GLMKeepEvalAreaStatsAll.df4$BEEDiffPercENMCurrArea <-  DiffBEEPercProjHistEcoregion
    GLMKeepEvalAreaStatsAll.df4$ProjENMArea <- ProjEcoregionProjPresArea
    GLMKeepEvalAreaStatsAll.df4$ProjPercENMArea <- ProjPercEcoregion
    GLMKeepEvalAreaStatsAll.df4$EcoRegNum <- EcoRegNum
    GLMKeepEvalAreaStatsAll.df4$Climate <- "Current"
    GLMKeepEvalAreaStatsAll.df4 <-GLMKeepEvalAreaStatsAll.df4[,c(29:30,1:28)]
    ## Save output to list
    Count <- Count + 1
    GLMKeepEvalStatsL[[Count]] <- GLMKeepEvalAreaStatsAll.df4
  }
}
t2 <- Sys.time()
#############################################################################
...
## Save data
setwd(OutDirectpsa)
write.table(GLMKeepEvalStatsAll.df, file=paste0(Species, "EcoregionCurrentFireAreaData.csv"), sep=",", col.names=NA)
#
...
# Find variable subsets for top ranked three models by regional indices of projections for ModelType
GLMKeepEvalAreaStatsAll.df <- data.frame(read.csv(paste0(Species, "GLM", DataSet, "_TrainVsTestandAreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
head(GLMKeepEvalAreaStatsAll.df)
# Keep only rankings for model type
GLMKeepEvalAreaStatsAll.df2 <- GLMKeepEvalAreaStatsAll.dfs[which(GLMKeepEvalAreaStatsAll.dfs$DataType==paste0("TopAUC", FullNObs, "FinalTest")),]
FinalModelRowNumberList.df <- as.data.frame(GLMKeepEvalAreaStatsAll.df2$ModelNumber)
FinalModelRowNumberList <- FinalModelRowNumberList.df[1:FinalNumberModels,]
...
GLMKeepEvalAreaStatsAll.df3 <- GLMKeepEvalAreaStatsAll.df2[1:4,]
ncol(GLMKeepEvalAreaStatsAll.df3)
### Read in GLM model ENM area projections for current climate
GLMCurrentClimateProjStats.df <- data.frame(read.csv(paste0(Species, "EcoregionCurrentFireAreaData.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
## Convert NA values to zero
GLMCurrentClimateProjStats.df[is.na(GLMCurrentClimateProjStats.df)] <- 0
#
GLMKeepEvalStatsL <- list()
##############################################################################
Count <- 0
    ...
    for(FinalModelRowNumber in FinalModelRowNumberList) {
      #FinalModelRowNumber=17
       ...
       for(FutClimSubDir in FutClimSubDirList) {
        ...
        ##
        setwd(OutDirectSub)
        ## Read in csv files of GLM grid cell counts inside Ecoregion
        ...
        ## Calculate percent of current projection relative to historic percent of ecoregion
        CurrProjPercEcoregion <- GLMCurrentClimateProjStats.df[(GLMCurrentClimateProjStats.df$ModelNumber==FinalModelRowNumber & GLMCurrentClimateProjStats.df$EcoRegNum==EcoRegNum),30]
        DiffPercProjFutCurrEcoregion <- FutProjPercEcoregion - CurrProjPercEcoregion
        ## Add data to evaluation statistics
        ## Add Breeding Range Area in Megahectares
        GLMKeepEvalAreaStatsAll.df4$ProjENMArea <- EcoregionFutProjPresArea
        GLMKeepEvalAreaStatsAll.df4$ProjPercENMArea <- FutProjPercEcoregion
        GLMKeepEvalAreaStatsAll.df4$ProjDiffPercFutCurrENMCurrArea <-  DiffPercProjFutCurrEcoregion
        GLMKeepEvalAreaStatsAll.df4$EcoRegNum <- EcoRegNum
        GLMKeepEvalAreaStatsAll.df4$Climate <- FutClimSubDir
        GLMKeepEvalAreaStatsAll.df4 <-GLMKeepEvalAreaStatsAll.df4[,c(27:28,1:26)]
        ## Save output to list
        Count <- Count + 1
        GLMKeepEvalStatsL[[Count]] <- GLMKeepEvalAreaStatsAll.df4
      }
...
#
setwd(OutDirectpsa)
write.table(GLMKeepEvalStatsFuture.df, file=paste0(Species, "EcoregionFutureFireAreaData.csv"), sep=",", col.names=NA)
###################################################################################
...
#FinalModelRowNumberList <- c(9,4,3)
# Read in regional indices for current models
GLMAreaStatsCurrent.df <- data.frame(read.csv(paste0(Species, "EcoregionCurrentFireAreaData.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
head(GLMAreaStatsCurrent.df)
# Add columns to match future data
GLMAreaStatsCurrent.df$ProjDiffPercFutCurrENMCurrArea <- 0
# Read in regional indices for future models
GLMAreaStatsFuture.df <- data.frame(read.csv(paste0(Species, "EcoregionFutureFireAreaData.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
head(GLMAreaStatsFuture.df)
# Add columns to match current data
GLMAreaStatsFuture.df$BEEENMArea <- 0
GLMAreaStatsFuture.df$BEEPercENMArea <- 0
GLMAreaStatsFuture.df$BEEDiffPercENMCurrArea <- 0
ncol(GLMAreaStatsFuture.df)
# Rearrange columns to match current data
GLMAreaStatsFuture.df <- GLMAreaStatsFuture.df[,c(1:25, 29:31, 26:28)]
# Combine current and future regional indices together
GLMAreaStatsClimate.df <- rbind(GLMAreaStatsCurrent.df, GLMAreaStatsFuture.df)
GLMAreaStatsClimate.dfs <- arrange(GLMAreaStatsClimate.df, Climate, DataType, ModelNumber, EcoRegNum, Climate)
head(GLMAreaStatsClimate.dfs)
# Create ClimateEcoregionType combining Climate and EcoRegNum
GLMAreaStatsClimate.dfs$ClimEcoRegType <- paste0(GLMAreaStatsClimate.dfs$Climate, "_", GLMAreaStatsClimate.dfs$EcoRegNum)
ncol(GLMAreaStatsClimate.dfs)
# Rearrange columns
GLMAreaStatsClimate.dfs <- GLMAreaStatsClimate.dfs[,c(1:4,32,5:31)]
head(GLMAreaStatsClimate.dfs)
ncol(GLMAreaStatsClimate.dfs)
#str(GLMAreaStatsClimate.dfs)
## Save combined data
write.table(GLMAreaStatsClimate.dfs, file=paste0(Species, "GLMCurrentFutureClimate", "_EcoregionAreaStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", "_TopVsRandom", NumProjections, "Sets.csv"), sep=",", col.names=NA)
###
### Calculate Mean and Standard Deviation Values
EvaluationStats <- GLMAreaStatsClimate.dfs
nrow(EvaluationStats)
OutName <- paste0(Species, "GLMProjection_Top4CurrFutModelEcoregionAreaStats_", SubsetVariableNumber, "Vars_", "_Summary.csv")
...
head(EvalStatsOuts)
write.table(EvalStatsOuts, file=paste0(Species, "GLMProjection_Top4CurFutModelEcoregionAreaStats_", SubsetVariableNumber, "Vars_", "_SummaryTable.csv"), sep=",", col.names=NA)
...
for(StatType in StatTypeNames) {
  ...
  for(ModelType in ModelTypeNames) {
    ...
    Count <- 0
    ## Keep only ModelType data
    GLMTestStats2 <- GLMAreaStatsClimate.dfs[which(GLMAreaStatsClimate.dfs$Set==ModelType),]
    head(GLMTestStats2)
    tail(GLMTestStats2)
    for(EcoRegNum in 1:53) {
      #EcoRegNum <- 1
      ## Keep only EcoRegNum
      GLMTestStats1 <- GLMTestStats2[which(GLMTestStats2$EcoRegNum==EcoRegNum),]
      head(GLMTestStats1)
      ## Use Welch correction one way ANOVA for unequal variances for primary test among all possible climate comparisons
      WelchANOVA <- oneway.test(as.formula(paste0(StatType, " ~ ClimEcoRegType")), data=GLMTestStats1, na.action=na.omit, var.equal=FALSE)
      # Save above Welch's ANOVA output
...
####################################################################################
### Calculate the GLM model area for the entire Projection Area for
...
#############################################################################
setwd(InDirect)
GLMProjectionAreaData.df <- as.data.frame(t(as.matrix(c(Species,0,0,0,0))), stringsAsFactors=FALSE)
colnames(GLMProjectionAreaData.df) <- c("Species", "AreaType", "SubsetVariableNumber", "ModelNum", "MxtProjArea")
GLMProjectionAreaData.df[,3:5] <- as.numeric(as.character(GLMProjectionAreaData.df[,3:5]))
Count <- 0
  ...
  for(AreaType in AreaTypeList) {
    for(NumVar in NumVarList)  {
      for(Model in ModelNumberList){
        ## Read in csv file with area of GLM model in projection area
        GLMProjectionArea.df <- data.frame(read.csv(paste0(Species, Model, "Mxt", NumVar, "bProjPresAreaTable", AreaType, ".csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
        ## Calculate Megahectares presence area within Ecoregion
        if(nrow(GLMProjectionArea.df)==0) {
          MxtProjresArea <- 0
        } else {
          MxtProjPresArea <- (GLMProjectionArea.df$AREA * 0.0001)/ 1000000 # convert from sq. meters to hectares then megehectares
        }
        ### Save data to dataframe
        Count <- Count + 1
        GLMProjectionAreaData.df[Count,1] <- Species
        GLMProjectionAreaData.df[Count,2] <- AreaType
        GLMProjectionAreaData.df[Count,3] <- NumVar
        GLMProjectionAreaData.df[Count,4] <- Model
        GLMProjectionAreaData.df[Count,5] <- as.numeric(MxtProjPresArea)
      }
    }
  }
}
...
## Save data
setwd(OutDirectpsa)
#write.table(GLMProjectionAreaData.df, file=paste0(Species, "GLMProjectionAreaData.csv"), sep=",", col.names=NA)
write.table(GLMProjectionAreaData.df, file=paste0(Species, "GLM15VarProjectionAreaData.csv"), sep=",", col.names=NA)
##########
###Calculate Mean and Standard Deviation Values
#
EvaluationStats <- GLMProjectionAreaData.df
ncol(EvaluationStats)
EvaluationStats$Model <- "GLM"
