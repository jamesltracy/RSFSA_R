...
source("GLMBinomialQuadMultiSubset_WrapperTrainEvalAICc_Function.R")
source("GLMBinomialQuadMultiSubset_WrapperTestEvalAICc_Function.R")
source("GLMBinomialQuadMultiSubset_WrapperTrainTestEvalAICc_Function.R")
source("GLMBinomialQuadMultiSubset_FinalTrainTestEvalAICc_Function.R")
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  system.time(GLMFiltSubsetEvalStats.df <- GLMBinomialQuadMultiSubset.WrapperTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn))
  #
  ######################
  ...
  for (j in 1:2) {
    ...
    ##
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GLMTrainSubsetEvalStats.df <- GLMBinomialQuadMultiSubset.WrapperTrainEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, DataSetType=DataSetType))
  ...
  for (j in 1:2) {
    ...
    DataSetType <- DataSetTypes[j]
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GLMTrainTestSubsetEvalStats.df <- GLMBinomialQuadMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, DataSetType=DataSetType))
    ## Separate out training and testing data
...
## Loop takes 39-75 minutes for 9,000 NumberModSets of 15 variables for glm bionomial quadratic low burn severity
###############################################################################
OutCount <- 0
##
GLMKeepEvalAllStatsL <- list()
...
#    ####################
    for(SetRun in SetRunsList) {
      ...
      SetCount <- SetCount + 1
      SetRunID <- SetCount
      system.time(GLMWrapperTrainTestSubsetEvalStats.df1 <- GLMBinomialQuadMultiSubset.WrapperTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, FunctDirectIn))
    }
    ##############################
...
#    setwd(OutDirect2Orig)
    ...
    for(ChosenModSets in ChosenModSetSizes) {
      ...
      ## Omit next two lines if using old data sets
      #RandNObsSubsets <- data.frame(GLMRandNObsFiltSubsetEvalStats.df[,1], stringsAsFactors=FALSE)
      #colnames(RandNObsSubsets) <- "VarNames"
      #################
      ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
      ...
      for (j in 1:length(DataSetTypes)) {
        ...
        DataSetType <- DataSetTypes[j]
        ## Evaluate random subsets for NumberModSets number of subsets
        system.time(GLMTrainTestSubsetEvalStats.df1 <- GLMBinomialQuadMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
        # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
