...
#source("GlmnetBinomialQuadOrthSubset_GridTrainTestEvalAICc_AUCpa_Calib_Function.R")
#source("GlmnetSubset_TrainTestEvalK_Function.R")
source("GlmnetBinomialQuadOrthMultiSubset_WrapperTrainEvalAICc_Function.R")
source("GlmnetBinomialQuadOrthMultiSubset_WrapperTestEvalAICc_Function.R")
source("GlmnetBinomialQuadOrthMultiSubset_WrapperTrainTestEvalAICc_Function.R")
source("GlmnetBinomialQuadOrthMultiSubset_FinalTrainTestEvalAICc_Function.R")
...
## Loop takes 62 min for Glmnet Binomial Quadratic Orthgonal
####################################################################################
t1 <- Sys.time()
...
for(SubsetLoop in SubsetSizes) {
  ...
  ### Execute only if reading in VariableSubsetsIn from another RSFSA run (e.g., MaxEnt)
    ...
    OutDirectpsaOld <- gsub("SWNAR3O", "SWNAR3", OutDirectpsaOld)
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  system.time(GlmnetFiltSubsetEvalStats.df <- GlmnetBinomialQuadOrthMultiSubset.WrapperTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa,  polycoefsall.df, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn))
  #
  ######################
...
#  colnames(RandNObsSubsets) <- "VarNames"
  ...
  for (j in 1:2) {
    ...
    ##
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GlmnetTrainSubsetEvalStats.df <- GlmnetBinomialQuadOrthMultiSubset.WrapperTrainEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa,  polycoefsall.df, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
  ...
  for (j in 1:2) {
    ...
    DataSetType <- DataSetTypes[j]
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GlmnetTrainTestSubsetEvalStats.df <- GlmnetBinomialQuadOrthMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa,  polycoefsall.df, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
    ## Separate out training and testing data
...
OutDirectpsaGLMAUC <- gsub("SWNAR3O", "SWNAR4", OutDirectpsaGLMAUC)
...
## Loop takes 7.2 hours for 9,000 NumberModSets of 15 variables for orthogonal glmnet bionomial quadratic low burn severity
###############################################################################
OutCount <- 0
##
#
...
#    ##
    ...
    for(SetRun in SetRunsList) {
      ...
      SetCount <- SetCount + 1
      SetRunID <- SetCount
      system.time(GlmnetWrapperTrainTestSubsetEvalStats.df1 <- GlmnetBinomialQuadOrthMultiSubset.WrapperTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn))
    }
    ###########################
...
#    setwd(OutDirect2Orig)
    ...
    for(ChosenModSets in ChosenModSetSizes) {
      ...
      ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
        ...
        setwd(OutDirect2)
       ##################
      ### Comment next line if reading in previous subsets
      #RandNObsSubsets <- data.frame(GlmnetRandNObsFiltSubsetEvalStats.df[,1], stringsAsFactors=FALSE)
      colnames(RandNObsSubsets) <- "VarNames"
      # Use difference between evaluation statistics between train and test data to evaluate overfitting
      ###############################################################################
      ...
      for (j in 1:length(DataSetTypes)) {
        ...
        DataSetType <- DataSetTypes[j]
        ## Evaluate random subsets for NumberModSets number of subsets
        system.time(GlmnetTrainTestSubsetEvalStats.df1 <- GlmnetBinomialQuadOrthMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, polycoefsall.df, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
        # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
...
#    write.table(data.frame(NObskfoldgrpa), file=paste0(Species, PsAbsBuffs, "PseudoabsenceDat", NObsJ, "Kfold", ".csv"), sep=",")
  ...
  #  names(Predictors) <- gsub("_NS", "", names(Predictors))
    ...
    CRS.In <- CRS.WGS84
    #
    Time <- system.time(GlmnetKeepEvalStats.df <- GlmnetBinomialQuadOrthSubset.GridTrainTestEvalAIC_AUCpa_Calib(Species, VariableNamesIn, PredictorsIn, SubsetVariableNumber, TotPres, Run, SetName, VariableSubset, PresAbsDat.df, BackgroundDat.df, kfoldgrppain, polycoefsall.df, cvglmnetfoldsin, CRS.In, OutDirectIn, FunctDirectIn,  Output=TRUE))
    #
    head(GlmnetKeepEvalStats.df)
    # Save results
    setwd(OutDirectIn)
