...
#
OutName <- paste0(Species, RankStatistic, "Glmnet_VariousSetsof_", TotVars, "_", NObs, "N_FinalTestStatSummary.csv")
...
#    write.table(data.frame(NObskfoldgrpa), file=paste0(Species, PsAbsBuffs, "PseudoabsenceDat", NObsJ, "Kfold", ".csv"), sep=",")
  ...
  #  names(Predictors) <- gsub("_NS", "", names(Predictors))
    ...
    CRS.In <- CRS.WGS84
    #
    Time <- system.time(GlmnetKeepEvalStats.df <- GlmnetBinomialQuadSubset.GridTrainTestEvalAIC_AUCpa_Calib(Species, VariableNamesIn, PredictorsIn, SubsetVariableNumber, TotPres, Run, SetName, VariableSubset, PresAbsDat.df, BackgroundDat.df, kfoldgrppain, cvglmnetfoldsin, CRS.In, OutDirectIn, FunctDirectIn,  Output=TRUE))
    #
    head(GlmnetKeepEvalStats.df)
    # Save results
    setwd(OutDirectIn)
