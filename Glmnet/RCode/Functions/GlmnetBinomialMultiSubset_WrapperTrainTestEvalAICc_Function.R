GlmnetBinomialMultiSubset.WrapperTrainTestEvalAICc <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn..., CVGlmnetRuns, Output, DataSetType) {
  setwd(OutDirectIn)
  library(foreach)
  library(doParallel)
...
#  GlmnetSubsetEvalStats.df <- GlmnetSubsetEvalStats.df[1:NumberModSets,]
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      write.table(GlmnetSubsetEvalStats.df, file=paste0(Species, "GlmnetResults_WrapperTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    } else {
      write.table(GlmnetSubsetEvalStats.df, file=paste0(Species, "GlmnetResults_WrapperTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    }
    #
