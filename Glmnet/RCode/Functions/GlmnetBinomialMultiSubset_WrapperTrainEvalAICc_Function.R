GlmnetBinomialMultiSubset.WrapperTrainEvalAICc <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn..., DataSetType, CVGlmnetRuns, Output) {
  setwd(OutDirectIn)
  library(foreach)
  library(doParallel)
  ...
  #
  ModelType1 <- paste("GlmnetBinomial", Subset, sep="")
  Model <- paste("GlmnetBinomial for", SubsetVariableNumber, "Feature Subset")
  ...
  GlmnetPresTrainData <- PresenceDat.df[kfoldgrpp==1,]
  GlmnetPresTrainData$Count <- 1
  head(GlmnetPresTrainData)
  #str(GlmnetPresTrainData)
  GlmnetAbsTrainData <- PseudoabsenceDat.df[kfoldgrpp==1,]
  ...
  GlmnetSubsetEvalStats <- foreach(i=ivec, .combine='rbind') %dopar%  {
    ...
    for(q in 1:CVGlmnetRuns) {
      ...
      head(xTrainData.df)
      nrow(xTrainData.df)
      xTrainData <- as.matrix(xTrainData.df)
      #####################
      ...
      #
      cv_glmnetBinomialModel = cv.glmnet(xTrainData, yTrainData, foldid=NTrainkfoldgrppa, family = "binomial", nfolds = cvglmnetfoldsin, alpha=alphain)
...
#    }
    ...
    ##
    #checkModel <- readRDS("cv_glmnetBinomialModel.rds")
    ...
    ## vals is vector of Glmnet raw presence values at occurence localities
    ...
    # Select only terms used in  model
...
#  GlmnetSubsetEvalStats.df <- GlmnetSubsetEvalStats.df[1:NumberModSets,]
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      write.table(GlmnetSubsetEvalStats.df, file=paste0(Species, "GlmnetResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, ".csv"), sep=",", col.names=NA)
    } else {
      write.table(GlmnetSubsetEvalStats.df, file=paste0(Species, "GlmnetResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, ".csv"), sep=",", col.names=NA)
    }
    #
