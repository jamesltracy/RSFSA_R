GlmnetBinomialMultiSubset.WrapperTestEvalAICc <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, NumberModSets, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn..., CVGlmnetRuns, Output) {
  library(foreach)
  library(doParallel)
  ...
  if(Output==TRUE) {
    ...
    write.table(GlmnetSubsetEvalStats.df, file=paste0(Species, "GlmnetResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    #
