...
source("GlmnetBinomialQuadSubset_GridTrainTestEvalAICc_AUCpa_Calib_Function.R")
#source("GlmnetSubset_TrainTestEvalK_Function.R")
source("GlmnetBinomialQuadMultiSubset_WrapperTrainEvalAICc_Function.R")
source("GlmnetBinomialQuadMultiSubset_WrapperTestEvalAICc_Function.R")
source("GlmnetBinomialQuadMultiSubset_WrapperTrainTestEvalAICc_Function.R")
source("GlmnetBinomialQuadMultiSubset_FinalTrainTestEvalAICc_Function.R")
...
ModelName <- paste0("GlmnetBinQuad", DataSet, "_", AnalysisType)
...
#SubsetSizes <- c(10,12,15,20,25)
#SubsetSizes <- c(10)
length(SubsetSizes)
OutCount <- 0
## Loop takes 80 to 87 min on new laptop
## Takes 4.8 to 5.2 hrs for Glmnet Binomial Quadratic
####################################################################################
t1 <- Sys.time()
...
for(SubsetLoop in SubsetSizes) {
  ...
  ### Execute only if reading in VariableSubsetsIn from another RSFSA run (e.g., MaxEnt)
    OutDirectpsaOld <- gsub("GlmnetBinQuad", "Maxent", OutDirectpsa)
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  system.time(GlmnetFiltSubsetEvalStats.df <- GlmnetBinomialQuadMultiSubset.WrapperTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn))
  #
  ######################
...
#  colnames(RandNObsSubsets) <- "VarNames"
  ...
  ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
    ...
    # Subset GlmnetFiltSubsetsEvalStats.df2 by RandNObsSubsets
    GlmnetRandNObsFiltSubsetEvalStats.df <- GlmnetFiltSubsetEvalStats.df2[GlmnetFiltSubsetEvalStats.df2$VarNames %in% RandNObsSubsets[,1],]
  ##################
  ...
  for (j in 1:2) {
    ...
    ##
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GlmnetTrainSubsetEvalStats.df <- GlmnetBinomialQuadMultiSubset.WrapperTrainEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
  ...
  for (j in 1:2) {
    ...
    DataSetType <- DataSetTypes[j]
    ## Evaluate random subsets for NumberModSets number of subsets
    system.time(GlmnetTrainTestSubsetEvalStats.df <- GlmnetBinomialQuadMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
    ## Separate out training and testing data
...
OutDirectpsaGLMAUC <- gsub("GlmnetBinQuad", "GLMBinomial", OutDirectpsa)
OutDirectpsaGLMAUC <- gsub("SWNAR3", "SWNAR4", OutDirectpsaGLMAUC)
...
setwd(OutDirectpsa)
GlmnetKeepEvalStats.df <- data.frame(read.csv(paste0(Species, ModelName, NObs, "_FullSetTrainVsTestStats_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff.csv"), stringsAsFactors=FALSE))
head(GlmnetKeepEvalStats.df)
## Calculate AICc_bg_reg using regression equation
xDatIn <- data.frame(AICc2 = GlmnetKeepEvalStats.df$AICc2)
head(xDatIn)
GlmnetKeepEvalStats.df$AICc_bg_reg <- predict(fitAIC, newdata=xDatIn)
head(GlmnetKeepEvalStats.df)
tail(GlmnetKeepEvalStats.df)
# Drop first column of row names
GlmnetKeepEvalStats.df <- GlmnetKeepEvalStats.df[,c(-1)]
ncol(GlmnetKeepEvalStats.df)
# Move the NumDVars and NumEnVars to end
GlmnetKeepEvalStats.df2 <- GlmnetKeepEvalStats.df[,c(1:4, 6:18, 20, 5, 19)]
head(GlmnetKeepEvalStats.df2)
## Save revised data
setwd(OutDirectpsa)
write.table(GlmnetKeepEvalStats.df2, file=paste0(Species, ModelName, NObs, "_FullSetTrainVsTestStats_", min(SubsetSizes), "to", max(SubsetSizes), "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff2.csv"), sep=",", row.names=FALSE)
...
GlmnetKeepEvalStatsNewSummary.df <- data.frame(read.csv(paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, "2.csv"), stringsAsFactors=FALSE))
head(GlmnetKeepEvalStatsNewSummary.df)
GlmnetKeepEvalStatsNewSummary.dfs <- arrange(GlmnetKeepEvalStatsNewSummary.df, DataType)
# Drop first column of row names
GlmnetKeepEvalStatsNewSummary.dfs <- GlmnetKeepEvalStatsNewSummary.dfs[,c(-1)]
GlmnetKeepEvalStatsNewSummary.dfs[1:20,]
write.table(GlmnetKeepEvalStatsNewSummary.dfs, file=paste0(Species, ModelName, "_FullSet", "SummaryStats_of", min(SubsetSizes), "to", max(SubsetSizes), "Vars", FSAType,"_", RunType, "2.csv"), sep=",", col.names=NA)
...
#
## Loop takes 9.7 to 10.3 hours for 9,000 NumberModSets of 15 variables for glmnet bionomial quadratic low burn severity
###############################################################################
OutCount <- 0
##
GlmnetKeepEvalAllStatsL <- list()
...
#    setwd(OutDirect2Orig)
    ...
    for(ChosenModSets in ChosenModSetSizes) {
      ...
      for(m in 1:length(AUCrankList)) {
        ...
        tail(GlmnetFiltSubsetEvalStats.df5)
        nrow(GlmnetFiltSubsetEvalStats.df5)
      ...
      for (j in 1:length(DataSetTypes)) {
        #j=1
        VariableSubsetsIn <- data.frame(SubsetRuns[[j]], stringsAsFactors=FALSE)
        VariableSubsetsIn <- na.omit(VariableSubsetsIn)
        DataSetType <- DataSetTypes[j]
        ## Evaluate random subsets for NumberModSets number of subsets
        system.time(GlmnetTrainTestSubsetEvalStats.df1 <- GlmnetBinomialQuadMultiSubset.FinalTrainTestEvalAICc(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, cvglmnetfoldsin, alphain, OutDirectIn, FunctDirectIn, DataSetType=DataSetType))
        # Delete any subsets with "Inf" for AICc_bg (means too many parameters in model relative to observations)
...
setwd(OutDirectpsa)
# Read data back in and sort
GlmnetKeepEvalStats.df <- data.frame(read.csv(paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_TrainVsTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
nrow(GlmnetKeepEvalStats.df)
ncol(GlmnetKeepEvalStats.df)
head(GlmnetKeepEvalStats.df)
tail(GlmnetKeepEvalStats.df)
...
## Save revised data
setwd(OutDirectpsa)
write.table(GlmnetKeepEvalStats.df, file=paste0(Species, ModelName, FullNObs, "_", NumberModSets,"SetNumber_TrainVsTestStats_of", FinalModelVariables, "of", TotVars, "NumVars_", FSAType, PsAbsBuff, Units, "PsAbsBuff_", SpatFiltBuff, Units, "SpatFiltBuff2.csv"), sep=",", row.names=FALSE)

#######################################################################################
###Calculate Mean and Standard Deviation Values including AICcbg_reg
setwd(OutDirectpsa)
RunType <- "Final"
EvaluationStats <- GlmnetKeepEvalStats.df
ncol(EvaluationStats)
EvaluationStats$Model <- ModelName
...
#
# Keep only DataTypes including string "FinalTest"
GlmnetKeepEvalStats.df2 <- GlmnetKeepEvalStats.df[grep("FinalTest", GlmnetKeepEvalStats.df$DataType), ]
GlmnetKeepEvalStats.df2[1:50,]
head(GlmnetKeepEvalStats.df2)
tail(GlmnetKeepEvalStats.df2)
###Calculate Mean and Standard Deviation Values
    ...
    for(RankType in RankTypeNames) {
      ...
      for(RandomType in RandomTypeNames) {
        ...
        if(StatType==StatTypeNames[1]) {
          ...
          GlmnetTestStats <- GlmnetTestStats[,c(1:9, 22, 10:21)]
...
## This loop takes 27 minutes for 9 models of 15 variable subsets for fire using glmnet
#
...
####
## First read in linear model predicting AICc_bg_reg
OutDirectpsaGLMAUC <- gsub("GlmnetBinQuad", "GLMBinomial", OutDirectpsa)
OutDirectpsaGLMAUC <- gsub("SWNAR3", "SWNAR4", OutDirectpsaGLMAUC)
setwd(OutDirectpsaGLMAUC)
fitAICStats.df <- data.frame(read.csv(paste0(Species, "_AllWrapperTest_AICc_bg_Vs_AICc2_RegressionStats.csv"), stringsAsFactors=FALSE))
...
#    write.table(data.frame(NObskfoldgrpa), file=paste0(Species, PsAbsBuffs, "PseudoabsenceDat", NObsJ, "Kfold", ".csv"), sep=",")
    ...
    #VariableSubset <- data.frame(VariableSubsetsIn[1,1], stringsAsFactors=FALSE)
  ...
  #  names(Predictors) <- gsub("_NS", "", names(Predictors))
    ...
    CRS.In <- CRS.WGS84
    #
    Time <- system.time(GlmnetKeepEvalStats.df <- GlmnetBinomialQuadSubset.GridTrainTestEvalAICc_AUCpa_Calib(Species, VariableNamesIn, PredictorsIn, SubsetVariableNumber, TotPres, Run, SetName, VariableSubset, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrppin, kfoldgrpain, cvglmnetfoldsin, CRS.In, OutDirectIn, FunctDirectIn,  Output=TRUE, fitAICModel=fitAIC))
    #
    head(GlmnetKeepEvalStats.df)
    # Save results
    setwd(OutDirectIn)
    write.table(GlmnetKeepEvalStats.df , file=paste0(Species, "Glmnet", DataSet, "_TrainVsTestThreshCal_of", SubsetVariableNumber, "Vars_", Loop, ".csv"), sep=",", col.names=NA)
    #
    ...
    GlmnetKeepEvalStats.df$ModelNumber <- Loop
    ncol(GlmnetKeepEvalStats.df)
    # Re-order columns
    GlmnetKeepEvalStats.df <- GlmnetKeepEvalStats.df[,c(1:4, 19:20, 5:18)]
    # Replace Run with Rep
    GlmnetKeepEvalStats.df$Rep <- Rep
    ncol(GlmnetKeepEvalStats.df)
    GlmnetKeepEvalStats.df <- GlmnetKeepEvalStats.df[,c(1:8, 21, 10:20)]
...
setwd(OutDirectpsa)
GlmnetKeepEvalAreaStatsAll.dfs <- data.frame(read.csv(paste0(Species, "Glmnet", DataSet, "_TrainVsTestStatsThreshCal_of", NumberModSets, "_", SetNameF, SubsetVariableNumber, "_", EvalType, "_TopVsRandom", NumProjections, "Sets.csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
head(GlmnetKeepEvalAreaStatsAll.dfs)
# Keep only rankings for model type
GlmnetKeepEvalAreaStatsAll.df2 <- GlmnetKeepEvalAreaStatsAll.dfs[which(GlmnetKeepEvalAreaStatsAll.dfs$DataType=="Test"),]
head(GlmnetKeepEvalAreaStatsAll.df2)
  ...
  for(AreaType in AreaTypeList) {
    for(NumVar in NumVarList)  {
      for(Model in ModelNumberList){
        ...
        GlmnetProjectionArea.df <- data.frame(read.csv(paste0(Species, Model, "glm", NumVar, "bProjPresAreaTable", AreaType, ".csv"), header = TRUE, row.names=1, sep=','), stringsAsFactors=FALSE)
        ## Calculate Megahectares presence area within Ecoregion
