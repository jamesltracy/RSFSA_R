MaxentMultiSubset.WrapperTrainEvalAIC <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, WrapperTrainSWD, WrapperTrainPresAbsID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn..., CatVarsPrefix, MaxentArgsIn, DataSetType, Output) {
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, ".csv"), sep=",", col.names=NA)
    } else {
      write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_Wrapper_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, ".csv"), sep=",", col.names=NA)
    }
