MaxentSubset.TrainTestEval <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, Run, SetName, VariableSubset, TrainSWD, TrainPresAbsID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrppin, kfoldgrpain, OutDirectIn..., CatVarsPrefix, MaxentArgsIn, LongOutDirect, Output) {
  ...
  #
  ModelType <- paste0("Maxent", SetName, "_", Run)
  ...
  for(k in 1:2) {
    ...
    MaxentEvalStats.df <- data.frame(t(as.matrix(c(VariableNamesSel, SubsetVariableNumber, TestDataType, TotPres, SetName, Run, accusum.df$MaxTSS, accusum.df$MaxKappa, accusum.df$AUC, accusum.df$Specificity_maxTSS, accusum.df$Sensitivity_maxTSS, accusum.df$ThresholdMaxTSS))))
    #
  ...
  colnames(MaxentSubsetEvalStats.df1) <- c("VarNames", "SubsetVariableNumber", "DataType", "TotalPresPnts", "SetName", "Run", "TSS", "Kappa", "AUC", "Spec", "Sens", "ThreshMxTSS")
    ...
    MaxentSubsetEvalStats.df1[,6:12] <- sapply(MaxentSubsetEvalStats.df1[,6:12], function(x) as.numeric(as.character(x)))
    ...
    if(k==2) {
      ...
      MaxentSubsetEvalL[[3]][,7:12] <- MaxentSubsetEvalL[[1]][,7:12] - MaxentSubsetEvalL[[2]][,7:12]
      MaxentSubsetEvalL[[3]][,3] <- "Diff"
    }
  }
  #####################################################################################
  ...
  if(Output==TRUE) {
    ...
    write.table(MaxentSubsetEvalStats.df, file=paste(Species, "EnvScrResults_TrainTest_", TotVars, "_TotVars_", SubsetVariableNumber, "Vars_", Sets, Subset, "_", SetName, "_", Run, ".csv", sep=""), sep=",", col.names=NA)
  }
  #
  gc()
