MaxentSubset.FinalTrainTestEvalK <-
function(Species, VariableNames, SubsetVariableNumber, TotPres, Run, VariableSubset, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, TrainK, TestK, kfoldgrpp, kfoldgrpa, OutDirectIn..., CatVarsPrefix, MaxentArgsIn, LongOutDirect, Output) {
  ...
  if(SubsetVarNum == 1) {
    MaxentPresTrainData1 <- data.frame(MaxentPresTrainData[,4])
    colnames(MaxentPresTrainData1) <- colnames(MaxentPresTrainData)[4]
    head(MaxentPresTrainData1)
    MaxentAbsTrainData1 <- data.frame(MaxentAbsTrainData[,4])
    colnames(MaxentAbsTrainData1) <- colnames(MaxentAbsTrainData)[4]
    head(MaxentAbsTrainData1)
    FinalTrainSWD <- rbind(MaxentPresTrainData1, MaxentAbsTrainData1)
    head(TrainSWD)
  } else {
  ...
  TrainPresAbsID <- rbind(FinalTrainPresID, FinalTrainAbsID)
  head(TrainPresAbsID)
  tail(TrainPresAbsID)
  #
  ...
  MaxentOut <- maxent(MaxentTrainDataK, FinalTrainPresAbsID, args=MaxentArgs, path=OutDirectSub)
  #str(MaxentOut)
