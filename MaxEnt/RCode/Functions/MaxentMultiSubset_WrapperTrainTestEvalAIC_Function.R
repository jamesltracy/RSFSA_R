MaxentMultiSubset.WrapperTrainTestEvalAIC <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, SetRunID, WrapperTrainSWD, WrapperTrainPresAbsID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn..., CatVarsPrefix, MaxentArgsIn, DataSetType, Output) {
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    } else {
      write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_WrapperTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunID, ".csv"), sep=",", col.names=NA)
    }
    out5 <- capture.output(print(MaxentArgsIn))
    cat(out5, file=paste(Species, "MaxentWrapperTrainTestResultsSummary_", TotVars, "TotVars_", ".txt", sep=""), sep="\n", append=TRUE)
  }
  #
  stopCluster(cl)
  registerDoSEQ()
