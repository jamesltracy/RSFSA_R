MaxentMultiSubset.FinalTrainTestEvalAIC <-
function(Species, VariableNamesIn, SubsetVariableNumber, TotPres, VariableSubsetsIn, FinalTrainSWD, FinalTrainPresAbsID, PresenceDat.df, PseudoabsenceDat.df, BackgroundDat.df, kfoldgrpp, kfoldgrpa, OutDirectIn..., SetRunIDIn, CatVarsPrefix, MaxentArgsIn, DataSetType, Output) {
  ...
  if(Output==TRUE) {
    ...
    if(DataSetType!="") {
      if(SetRunIDIn!="") {
        write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_TrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", DataSetType, "_", Sets, "_", SetRunIDIn, ".csv"), sep=",", col.names=NA)
      } else {
      ...
      if(SetRunIDIn!="") {
        write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_FinalTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, "_", SetRunIDIn, ".csv"), sep=",", col.names=NA)
      } else {
        write.table(MaxentSubsetEvalStats.df, file=paste0(Species, "MaxentResults_FinalTrainTest_", TotVars, "TotVars_", SubsetVariableNumber, "Vars_", Sets, ".csv"), sep=",", col.names=NA)
      }
    }
    out5 <- capture.output(print(MaxentArgsIn))
    cat(out5, file=paste(Species, "MaxentFinalTrainTestResultsSummary_", TotVars, "TotVars_", ".txt", sep=""), sep="\n", append=TRUE)
  }
  #
  stopCluster(cl)
  registerDoSEQ()
