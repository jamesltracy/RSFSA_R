...
MaxentBaseArgsIn <- c(paste0("betamultiplier=", BetaMult), "product=false", "hinge=false", "threshold=false", "writebackgroundpredictions=true")
...
MaxentEvalArgs1 <- c(paste0("betamultiplier=", BetaMult), "product=false", "hinge=false", "threshold=false", "writebackgroundpredictions=true", "outputgrids=FALSE", "replicates=3", "replicatetype=crossvalidate", "responsecurves=true", "jackknife=true")
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  ...
  ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
    OutDirectpsaOld <- gsub("SWNAR3LQ", "SWNAR3", OutDirectpsa)
