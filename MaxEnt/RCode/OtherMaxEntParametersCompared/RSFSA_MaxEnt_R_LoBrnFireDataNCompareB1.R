...
length(SubsetSizes)
OutCount <- 0
## Loop takes 80 to 87 min on new laptop with BetaMult of 2
## Loop takes 55 minutes with BetaMult of 10
## Loop takes 47 minutes with BetaMult of 20
## Loop takes 67 minutes with BetaMult of 5
## Loop takes 80 minutes with BetaMult of 3
####################################################################################
t1 <- Sys.time()
...
for(SubsetLoop in SubsetSizes) {
  ...
  ### Execute only if reading in VariableSubsetsIn from another RSFSA run (e.g., MaxEnt)
    OutDirectpsaOld <- gsub(Tag, "R3", OutDirectpsa)
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  ...
  ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
    ...
    # Subset MaxentFiltSubsetsEvalStats.df2 by RandNObsSubsets
    MaxentRandNObsFiltSubsetEvalStats.df <- MaxentFiltSubsetEvalStats.df2[MaxentFiltSubsetEvalStats.df2$VarNames %in% RandNObsSubsets[,1],]
  ##################
...
## Loop takes 6.6 hours for 9,000 NumberModSets of 15 variables for BetaMult = 3 (above are BetaMult = 2) lo brn fire on new laptop
## Loop takes 8.8 hours for 9,000 NumberModSets of 15 variables for BetaMult = 1 (above are BetaMult = 2) lo brn fire on new laptop
###############################################################################
OutCount <- 0
##
MaxentKeepEvalAllStatsL <- list()
