...
BetaMult <- 2
## Specify base argumenets for Maxent settings for evaluation run
...
MaxentBaseArgsIn <- c(paste0("betamultiplier=", BetaMult), "quadratic=false", "product=false", "hinge=false", "threshold=false", "writebackgroundpredictions=true")
#MaxentBaseArgsIn <- c(paste0("betamultiplier=", BetaMult),  "writebackgroundpredictions=true")
#
...
MaxentEvalArgs1 <- c(paste0("betamultiplier=", BetaMult), "quadratic=false", "product=false", "hinge=false", "threshold=false", "writebackgroundpredictions=true", "outputgrids=FALSE", "replicates=3", "replicatetype=crossvalidate", "responsecurves=true", "jackknife=true")
...
## Took 37 minutes with only linear features 3/20/18
## Took 18 minutes with only linear features for AICc run 3/21/2018
####################################################################################
t1 <- Sys.time()
...
#  NOTE: The next line can be commented if just re-ranking the subsets by a different evaluation statistic
  ...
  ### Execute only if reading in RandNObsSubsets from another RSFSA run (e.g., MaxEnt)
    OutDirectpsaOld <- gsub("SWNAR3L", "SWNAR3", OutDirectpsa)
